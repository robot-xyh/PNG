...
STAMP="${STAMP:-mnp_fixed_vm_$(date +%Y%m%d_%H%M%S)}"
...
run_one() {
  ...
  echo "Running ${label}: range=${range_m}m, altitude_offset=${ALTITUDE_OFFSET}m, guidance=fixed_vm_png"
...
echo "Strict-reset M/N/P fixed-Vm PNG stamp: ${STAMP}"
echo "Ranges: ${RANGES[*]}"
echo "Every case restarts PX4 SITL and Blocks."
echo "Fixed guidance: N=${NAVIGATION_CONSTANT}, Vm=${SPEED_RATIO}*${INTRUDER_SPEED} m/s"

for range_m in "${RANGES[@]}"; do
  run_one M sensor_noise "$range_m"
done
summarize_label M

for range_m in "${RANGES[@]}"; do
  run_one N sensor_noise "$range_m" --no-los-filter
done
summarize_label N

for range_m in "${RANGES[@]}"; do
  run_one P no_sensor_noise "$range_m" --no-los-filter
done
summarize_label P

python3 examples/generate_mnp_strapdown_report.py --stamp "$STAMP"

echo
echo "mnp_stamp=${STAMP}"
echo "report=完整方案/MNP_SITL固定Vm比例导引识别对比报告.md"
