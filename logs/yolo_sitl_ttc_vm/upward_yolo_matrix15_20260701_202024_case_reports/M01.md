# YOLO+ByteTrack upward matrix M01
...
Matrix case M01: range=40m, lateral=-10m, altitude=30m, intruder_speed=5m/s, speed_ratio=2.0; AirSim detect shadow disabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_yolo_matrix15_20260701_202024_M01`|
...
![summary](M01_assets/summary_upward_yolo_matrix15_20260701_202024_M01.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/1|0/1|40|-|-|1.659|83/83|82/83|9.79|
|VM|1/1|0/1|40|-|-|1.854|78/81|80/81|9.80|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|40|1|0|11.18|-|-|1.659|1.659|100.0%|98.8%|9.79|7.90|0.84|1.01|0.71|
|VM|40|1|0|10.71|-|-|1.854|1.948|96.3%|98.8%|9.80|7.90|0.81|0.91|0.55|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![40m](M01_assets/yolo_sitl_ttc_vm_040m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|40|1.659|`valid`|valid:58, area_not_expanding:23, bbox_bottom_clipped:1, ttc_out_of_range:1|100.0%|98.8%|
|VM|40|1.854|`image_kf_predict`|valid:77, image_kf_predict:3, bbox_bottom_clipped:1|96.3%|98.8%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 本轮平均实际过载峰值约 `0.82 g`，平均需用过载 P95 约 `0.63 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](M01_assets/shadow_summary_upward_yolo_matrix15_20260701_202024_M01.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|40|1|1.659|100.0%|0/8|56.6|0.69|2.65|0.71|0.84|
|VM|40|1|1.854|66.7%|3/9|31.1|0.35|2.65|0.55|0.81|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|40|1.659|100.0%|2.4%|120.0/120.0/60.0|35.0/35.0|0.25/0.79|
|VM|40|1.854|100.0%|0.0%|86.7/120.0/51.0|22.4/35.0|0.36/0.79|
...
- TTC: 命中 `1/1`，命中距离 `40m`，未命中 `-`，检测帧比例 `100.0%`，有效导引帧比例 `98.8%`，平均检测 FPS `9.79`。
- VM: 命中 `1/1`，命中距离 `40m`，未命中 `-`，检测帧比例 `96.3%`，有效导引帧比例 `98.8%`，平均检测 FPS `9.80`。
