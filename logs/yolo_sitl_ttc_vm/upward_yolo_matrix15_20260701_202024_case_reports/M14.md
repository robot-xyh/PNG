# YOLO+ByteTrack upward matrix M14
...
Matrix case M14: range=55m, lateral=15m, altitude=20m, intruder_speed=7m/s, speed_ratio=2.0; AirSim detect shadow disabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_yolo_matrix15_20260701_202024_M14`|
...
![summary](M14_assets/summary_upward_yolo_matrix15_20260701_202024_M14.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|0/1|0/1|-|-|55|58.426|10/281|12/281|9.12|
|VM|0/1|0/1|-|-|55|9.418|36/281|40/281|9.22|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|55|0|0|-|-|-|58.426|498.803|3.6%|4.3%|9.12|7.92|0.83|0.00|0.00|
|VM|55|0|0|-|-|-|9.418|47.669|12.8%|14.2%|9.22|7.93|1.22|1.31|0.54|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![55m](M14_assets/yolo_sitl_ttc_vm_055m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|55|58.426|`no_detection`|no_detection:269, area_not_expanding:5, valid:3, image_kf_predict:2|3.6%|4.3%|
|VM|55|9.418|`no_detection`|no_detection:239, valid:33, image_kf_predict:6, bbox_bottom_clipped:2|12.8%|14.2%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 检测率低于 60% 的工况：TTC 55m(3.6%)，VM 55m(12.8%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 55m:`no_detection`，VM 55m:`no_detection`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `1.03 g`，平均需用过载 P95 约 `0.27 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](M14_assets/shadow_summary_upward_yolo_matrix15_20260701_202024_M14.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|55|0|58.426|0.0%|3/3|nan|0.02|0.88|0.00|0.83|
|VM|55|0|9.418|0.0%|15/15|108.0|0.28|2.04|0.54|1.22|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|55|58.426|6.8%|1.1%|120.0/120.0/60.0|35.0/35.0|0.25/0.76|
|VM|55|9.418|27.0%|2.1%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
...
- TTC: 命中 `0/1`，命中距离 `-`，未命中 `55m`，检测帧比例 `3.6%`，有效导引帧比例 `4.3%`，平均检测 FPS `9.12`。
- VM: 命中 `0/1`，命中距离 `-`，未命中 `55m`，检测帧比例 `12.8%`，有效导引帧比例 `14.2%`，平均检测 FPS `9.22`。
