# YOLO+ByteTrack upward matrix M13
...
Matrix case M13: range=45m, lateral=-20m, altitude=40m, intruder_speed=7m/s, speed_ratio=2.0; AirSim detect shadow disabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_yolo_matrix15_20260701_202024_M13`|
...
![summary](M13_assets/summary_upward_yolo_matrix15_20260701_202024_M13.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/1|1/1|45|45|-|0.945|104/109|107/109|9.69|
|VM|0/1|0/1|-|-|45|3.637|170/280|167/280|9.11|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|45|1|1|14.29|14.17|1.245|0.945|0.945|95.4%|98.2%|9.69|7.92|0.98|1.93|0.95|
|VM|45|0|0|-|-|-|3.637|49.803|60.7%|59.6%|9.11|7.92|1.51|1.98|2.04|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![45m](M13_assets/yolo_sitl_ttc_vm_045m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|45|0.945|`terminal_lost`|valid:90, area_not_expanding:11, image_kf_predict:3, terminal_lost:3|95.4%|98.2%|
|VM|45|3.637|`valid`|valid:160, no_detection:90, terminal_lost:28, image_kf_predict:2|60.7%|59.6%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 本轮平均实际过载峰值约 `1.24 g`，平均需用过载 P95 约 `1.49 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](M13_assets/shadow_summary_upward_yolo_matrix15_20260701_202024_M13.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|45|1|0.945|87.5%|1/8|40.1|0.44|1.56|0.95|0.98|
|VM|45|0|3.637|100.0%|0/15|19.6|0.25|3.06|2.04|1.51|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|45|0.945|94.5%|4.6%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
|VM|45|3.637|55.0%|19.6%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
...
- TTC: 命中 `1/1`，命中距离 `45m`，未命中 `-`，检测帧比例 `95.4%`，有效导引帧比例 `98.2%`，平均检测 FPS `9.69`。
- VM: 命中 `0/1`，命中距离 `-`，未命中 `45m`，检测帧比例 `60.7%`，有效导引帧比例 `59.6%`，平均检测 FPS `9.11`。
