# YOLO+ByteTrack upward matrix M10
...
Matrix case M10: range=30m, lateral=-10m, altitude=30m, intruder_speed=7m/s, speed_ratio=2.0; AirSim detect shadow disabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_yolo_matrix15_20260701_202024_M10`|
...
![summary](M10_assets/summary_upward_yolo_matrix15_20260701_202024_M10.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|0/1|0/1|-|-|30|1.903|234/278|227/278|8.82|
|VM|0/1|0/1|-|-|30|2.993|228/278|212/278|8.84|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|30|0|0|-|-|-|1.903|61.421|84.2%|81.7%|8.82|7.87|1.14|2.46|2.02|
|VM|30|0|0|-|-|-|2.993|13.610|82.0%|76.3%|8.84|7.85|1.17|2.04|2.04|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![30m](M10_assets/yolo_sitl_ttc_vm_030m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|30|1.903|`area_not_expanding`|valid:133, area_not_expanding:83, no_detection:42, terminal_lost:14|84.2%|81.7%|
|VM|30|2.993|`valid`|valid:207, terminal_lost:44, no_detection:27|82.0%|76.3%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 30m(1.903m)，VM 30m(2.993m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 30m:`area_not_expanding`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `1.15 g`，平均需用过载 P95 约 `2.03 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](M10_assets/shadow_summary_upward_yolo_matrix15_20260701_202024_M10.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|30|0|1.903|100.0%|0/15|37.5|0.38|4.00|2.02|1.14|
|VM|30|0|2.993|100.0%|0/15|15.4|0.23|2.77|2.04|1.17|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|30|1.903|60.4%|29.9%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
|VM|30|2.993|65.8%|43.9%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
...
- TTC: 命中 `0/1`，命中距离 `-`，未命中 `30m`，检测帧比例 `84.2%`，有效导引帧比例 `81.7%`，平均检测 FPS `8.82`。
- VM: 命中 `0/1`，命中距离 `-`，未命中 `30m`，检测帧比例 `82.0%`，有效导引帧比例 `76.3%`，平均检测 FPS `8.84`。
