# YOLO+ByteTrack upward matrix M12
...
Matrix case M12: range=50m, lateral=-10m, altitude=30m, intruder_speed=7m/s, speed_ratio=2.0; AirSim detect shadow disabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_yolo_matrix15_20260701_202024_M12`|
...
![summary](M12_assets/summary_upward_yolo_matrix15_20260701_202024_M12.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/1|0/1|50|-|-|1.724|92/95|92/95|9.42|
|VM|0/1|0/1|-|-|50|3.298|150/277|151/277|8.66|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|1|0|12.56|-|-|1.724|1.936|96.8%|96.8%|9.42|7.90|0.67|0.70|0.69|
|VM|50|0|0|-|-|-|3.298|64.376|54.2%|54.5%|8.66|7.82|1.22|1.88|1.90|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![50m](M12_assets/yolo_sitl_ttc_vm_050m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|50|1.724|`valid`|valid:73, area_not_expanding:16, no_detection:3, ttc_out_of_range:3|96.8%|96.8%|
|VM|50|3.298|`terminal_lost`|valid:141, no_detection:110, terminal_lost:19, image_kf_predict:4|54.2%|54.5%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 检测率低于 60% 的工况：VM 50m(54.2%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：VM 50m:`terminal_lost`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.95 g`，平均需用过载 P95 约 `1.29 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](M12_assets/shadow_summary_upward_yolo_matrix15_20260701_202024_M12.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|1|1.724|100.0%|0/9|25.3|0.38|1.56|0.69|0.67|
|VM|50|0|3.298|40.0%|9/15|95.5|0.30|3.65|1.90|1.22|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|50|1.724|96.8%|2.1%|120.0/120.0/43.2|34.8/35.0|0.25/0.85|
|VM|50|3.298|44.8%|13.0%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
...
- TTC: 命中 `1/1`，命中距离 `50m`，未命中 `-`，检测帧比例 `96.8%`，有效导引帧比例 `96.8%`，平均检测 FPS `9.42`。
- VM: 命中 `0/1`，命中距离 `-`，未命中 `50m`，检测帧比例 `54.2%`，有效导引帧比例 `54.5%`，平均检测 FPS `8.66`。
