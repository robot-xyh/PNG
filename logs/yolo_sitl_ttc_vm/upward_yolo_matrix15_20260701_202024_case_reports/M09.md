# YOLO+ByteTrack upward matrix M09
...
Matrix case M09: range=30m, lateral=-10m, altitude=30m, intruder_speed=3m/s, speed_ratio=2.0; AirSim detect shadow disabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_yolo_matrix15_20260701_202024_M09`|
...
![summary](M09_assets/summary_upward_yolo_matrix15_20260701_202024_M09.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/1|0/1|30|-|-|1.509|82/92|86/92|8.62|
|VM|1/1|1/1|30|30|-|1.197|81/86|84/86|8.29|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|30|1|0|12.49|-|-|1.509|1.622|89.1%|93.5%|8.62|7.74|0.76|1.57|0.51|
|VM|30|1|1|12.20|12.04|1.289|1.197|1.197|94.2%|97.7%|8.29|7.42|0.71|1.44|1.57|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![30m](M09_assets/yolo_sitl_ttc_vm_030m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|30|1.509|`terminal_lost`|valid:55, area_not_expanding:19, terminal_lost:13, image_kf_predict:4|89.1%|93.5%|
|VM|30|1.197|`terminal_lost`|valid:78, terminal_lost:4, image_kf_predict:3, no_detection:1|94.2%|97.7%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 本轮平均实际过载峰值约 `0.73 g`，平均需用过载 P95 约 `1.04 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](M09_assets/shadow_summary_upward_yolo_matrix15_20260701_202024_M09.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|30|1|1.509|66.7%|3/9|67.2|0.31|1.45|0.51|0.76|
|VM|30|1|1.197|85.7%|1/7|57.9|0.46|1.34|1.57|0.71|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|30|1.509|68.5%|2.2%|120.0/120.0/60.0|35.0/35.0|0.25/0.80|
|VM|30|1.197|66.3%|8.1%|120.0/120.0/60.0|35.0/35.0|0.25/0.80|
...
- TTC: 命中 `1/1`，命中距离 `30m`，未命中 `-`，检测帧比例 `89.1%`，有效导引帧比例 `93.5%`，平均检测 FPS `8.62`。
- VM: 命中 `1/1`，命中距离 `30m`，未命中 `-`，检测帧比例 `94.2%`，有效导引帧比例 `97.7%`，平均检测 FPS `8.29`。
