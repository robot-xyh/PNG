# YOLO+ByteTrack upward matrix M03
...
Matrix case M03: range=55m, lateral=-10m, altitude=30m, intruder_speed=5m/s, speed_ratio=2.0; AirSim detect shadow disabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_yolo_matrix15_20260701_202024_M03`|
...
![summary](M03_assets/summary_upward_yolo_matrix15_20260701_202024_M03.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/1|1/1|55|55|-|1.158|93/108|102/108|9.64|
|VM|0/1|0/1|-|-|55|6.718|64/281|58/281|9.14|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|55|1|1|14.09|14.09|1.158|1.158|1.158|86.1%|94.4%|9.64|7.91|0.86|1.29|1.81|
|VM|55|0|0|-|-|-|6.718|86.874|22.8%|20.6%|9.14|7.91|0.90|0.96|0.54|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![55m](M03_assets/yolo_sitl_ttc_vm_055m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|55|1.158|`image_kf_predict`|valid:71, area_not_expanding:17, image_kf_predict:9, no_detection:6|86.1%|94.4%|
|VM|55|6.718|`no_detection`|no_detection:215, valid:55, bbox_bottom_clipped:9, image_kf_predict:2|22.8%|20.6%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 检测率低于 60% 的工况：VM 55m(22.8%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：VM 55m:`no_detection`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.88 g`，平均需用过载 P95 约 `1.17 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](M03_assets/shadow_summary_upward_yolo_matrix15_20260701_202024_M03.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|55|1|1.158|62.5%|3/8|32.1|0.32|0.68|1.81|0.86|
|VM|55|0|6.718|0.0%|16/16|94.6|0.25|1.45|0.54|0.90|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|55|1.158|96.3%|11.1%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
|VM|55|6.718|35.2%|0.0%|120.0/120.0/60.0|35.0/35.0|0.40/0.77|
...
- TTC: 命中 `1/1`，命中距离 `55m`，未命中 `-`，检测帧比例 `86.1%`，有效导引帧比例 `94.4%`，平均检测 FPS `9.64`。
- VM: 命中 `0/1`，命中距离 `-`，未命中 `55m`，检测帧比例 `22.8%`，有效导引帧比例 `20.6%`，平均检测 FPS `9.14`。
