# YOLO+ByteTrack upward matrix M06
...
Matrix case M06: range=40m, lateral=20m, altitude=30m, intruder_speed=5m/s, speed_ratio=2.0; AirSim detect shadow disabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_yolo_matrix15_20260701_202024_M06`|
...
![summary](M06_assets/summary_upward_yolo_matrix15_20260701_202024_M06.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/1|1/1|40|40|-|1.052|120/136|124/136|9.27|
|VM|0/1|0/1|-|-|40|4.054|95/275|99/275|8.83|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|40|1|1|17.89|17.76|1.079|1.052|1.052|88.2%|91.2%|9.27|7.83|1.00|1.32|2.04|
|VM|40|0|0|-|-|-|4.054|79.593|34.5%|36.0%|8.83|7.78|1.02|0.83|0.51|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![40m](M06_assets/yolo_sitl_ttc_vm_040m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|40|1.052|`bbox_bottom_clipped`|valid:82, area_not_expanding:31, terminal_lost:8, image_kf_predict:5|88.2%|91.2%|
|VM|40|4.054|`no_detection`|no_detection:174, valid:92, image_kf_predict:6, bbox_bottom_clipped:3|34.5%|36.0%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 检测率低于 60% 的工况：VM 40m(34.5%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：VM 40m:`no_detection`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `1.01 g`，平均需用过载 P95 约 `1.28 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](M06_assets/shadow_summary_upward_yolo_matrix15_20260701_202024_M06.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|40|1|1.052|100.0%|0/8|37.8|0.22|0.53|2.04|1.00|
|VM|40|0|4.054|0.0%|15/15|103.4|0.11|1.78|0.51|1.02|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|40|1.052|92.6%|14.0%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
|VM|40|4.054|40.0%|2.9%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
...
- TTC: 命中 `1/1`，命中距离 `40m`，未命中 `-`，检测帧比例 `88.2%`，有效导引帧比例 `91.2%`，平均检测 FPS `9.27`。
- VM: 命中 `0/1`，命中距离 `-`，未命中 `40m`，检测帧比例 `34.5%`，有效导引帧比例 `36.0%`，平均检测 FPS `8.83`。
