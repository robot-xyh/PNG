# YOLO+ByteTrack upward matrix M11
...
Matrix case M11: range=50m, lateral=-10m, altitude=30m, intruder_speed=3m/s, speed_ratio=2.0; AirSim detect shadow disabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_yolo_matrix15_20260701_202024_M11`|
...
![summary](M11_assets/summary_upward_yolo_matrix15_20260701_202024_M11.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/1|1/1|50|50|-|1.276|107/113|110/113|9.35|
|VM|1/1|1/1|50|50|-|1.187|111/125|116/125|9.55|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|1|1|14.77|14.64|1.474|1.276|1.276|94.7%|97.3%|9.35|7.91|0.76|0.86|0.41|
|VM|50|1|1|16.26|16.14|1.276|1.187|1.187|88.8%|92.8%|9.55|7.91|0.80|1.02|1.44|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![50m](M11_assets/yolo_sitl_ttc_vm_050m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|50|1.276|`image_kf_predict`|valid:98, area_not_expanding:7, no_detection:3, image_kf_predict:3|94.7%|97.3%|
|VM|50|1.187|`terminal_lost`|valid:105, no_detection:9, image_kf_predict:5, bbox_bottom_clipped:3|88.8%|92.8%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 本轮平均实际过载峰值约 `0.78 g`，平均需用过载 P95 约 `0.93 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](M11_assets/shadow_summary_upward_yolo_matrix15_20260701_202024_M11.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|1|1.276|75.0%|2/8|28.2|0.40|0.61|0.41|0.76|
|VM|50|1|1.187|100.0%|0/8|44.0|0.35|0.77|1.44|0.80|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|50|1.276|80.5%|0.0%|120.0/120.0/30.3|35.0/35.0|0.31/0.71|
|VM|50|1.187|81.6%|6.4%|120.0/120.0/60.0|35.0/35.0|0.25/0.75|
...
- TTC: 命中 `1/1`，命中距离 `50m`，未命中 `-`，检测帧比例 `94.7%`，有效导引帧比例 `97.3%`，平均检测 FPS `9.35`。
- VM: 命中 `1/1`，命中距离 `50m`，未命中 `-`，检测帧比例 `88.8%`，有效导引帧比例 `92.8%`，平均检测 FPS `9.55`。
