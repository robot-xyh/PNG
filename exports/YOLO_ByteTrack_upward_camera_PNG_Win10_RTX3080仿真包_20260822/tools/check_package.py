...
def validate(runtime: bool = False) -> list[str]:
    ...
    try:
        ...
        if str(config.get("wsl_distribution")) != "PNG-PX4-Ubuntu20.04":
            errors.append("the dedicated WSL1 distribution name is invalid")
        requirements = config.get("environment_requirements", {})
        expected_requirements = {
            "windows_min_build": 19045,
            "nvidia_driver_min": "560.76",
            "gpu_name_hint": "RTX 3080",
            "gpu_min_vram_gib": 8,
            "gpu_min_compute_capability": "8.6",
            "cuda_fp16_required": True,
            "wsl_version": 1,
        }
        for key, expected in expected_requirements.items():
            if requirements.get(key) != expected:
                errors.append(f"environment_requirements.{key} must be {expected!r}")
    except Exception as exc:
        errors.append(f"scenario validation failed: {exc}")

    try:
        installer = (PACKAGE_ROOT / "tools" / "install_windows.ps1").read_text(encoding="utf-8")
        required_installer_tokens = (
            "v1.8.1-windows/Blocks.zip",
            "259463081",
            "47c526a5f0acff42c211d2479b9de9f10286a8162678ef19adc09040a23ca1db",
            "core/focal/20230630/ubuntu-focal-core-cloudimg-amd64-root.tar.gz",
            "23a0b488bf439da0d5776748b39f668f726129dcc57d7b21544f1706652e2082",
            "--version 1",
        )
        for token in required_installer_tokens:
            if token not in installer:
                errors.append(f"Win10 installer is missing pinned token: {token}")
        for forbidden in ("networkingMode=mirrored", "--set-default-version 2", "--set-version $Distro 2"):
            if forbidden in installer:
                errors.append(f"Win10 installer contains forbidden WSL2 configuration: {forbidden}")
    except Exception as exc:
        errors.append(f"Win10 installer validation failed: {exc}")
    ...
    if runtime:
        ...
        try:
            ...
            if not torch.cuda.is_available():
                ...
                probe = torch.ones((8, 8), device="cuda", dtype=torch.float16)
                probe = probe @ probe
                torch.cuda.synchronize()
                if not bool(torch.isfinite(probe).all().item()):
                    errors.append("runtime CUDA FP16 validation failed")
        except Exception as exc:
            errors.append(f"runtime CUDA validation error: {exc}")
    return errors
