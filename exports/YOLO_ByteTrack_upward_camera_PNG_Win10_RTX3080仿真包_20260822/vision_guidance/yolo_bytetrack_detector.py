...
def add_detector_args(parser: Any) -> None:
    ...
    parser.add_argument(
        "--airsim-image-transport",
        choices=("raw", "compressed"),
        default="raw",
        help="AirSim scene image transport. raw avoids PNG encode/decode overhead; compressed is explicit compatibility mode.",
    )
    parser.add_argument(
        "--yolo-half",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Use FP16 inference when the selected YOLO device is CUDA capable.",
    )
    parser.add_argument("--yolo-warmup-iterations", type=int, default=2)
...
def create_detection_provider(args: Any, airsim_module: Any = None):
    ...
    if source == "yolo_bytetrack":
        return YoloByteTrackDetector(
            ...
            image_transport=str(getattr(args, "airsim_image_transport", "raw") or "raw"),
            half=bool(getattr(args, "yolo_half", True)),
            warmup_iterations=int(getattr(args, "yolo_warmup_iterations", 2)),
...
class YoloByteTrackDetector:
    ...
    def __init__(
        ...
        *,
        ...
        self.image_transport = str(image_transport)
        ...
        self.half = self.half_requested and _device_uses_cuda(self.device, self.runtime_info)
    ...
    def detect(
        ...
        *,
        ...
        stats: dict[str, Any] = {
            ...
            "yolo_single_target_distance_px": "",
            "yolo_image_transport": self.image_transport,
            "yolo_half_requested": int(self.half_requested),
            "yolo_half_effective": int(self.half),
        }
    ...
    def warmup(self, *, client: Any, config: AirSimDetectionConfig) -> int:
        """Warm up model kernels without creating ByteTrack state."""

        if self.warmup_iterations <= 0:
            return 0
        image_bgr = self._read_scene_image(client, config)
        if image_bgr is None:
            raise RuntimeError("YOLO warm-up failed: AirSim scene image is unavailable")
    ...
    def _read_scene_image(self, client: Any, config: AirSimDetectionConfig) -> Optional[np.ndarray]:
        ...
        return _read_airsim_scene_image(
            client,
            config,
            self.airsim_module,
            self.cv2,
...
def _read_airsim_scene_image(
    ...
    *,
    ...
    if transport == "compressed":
        ...
        return None if image is None else _ensure_bgr(image, cv2)
    ...
    if width <= 0 or height <= 0 or payload is None:
        return None
    if isinstance(payload, str):
        payload = payload.encode("latin1")
    raw = np.frombuffer(payload, dtype=np.uint8)
    pixel_count = width * height
    if pixel_count <= 0 or raw.size not in {pixel_count * 3, pixel_count * 4}:
        return None
    channels = raw.size // pixel_count
    image = raw.reshape(height, width, channels)
    if channels == 4:
        code = getattr(cv2, "COLOR_RGBA2BGR", None)
        return cv2.cvtColor(image, code) if code is not None else image[:, :, [2, 1, 0]]
    code = getattr(cv2, "COLOR_RGB2BGR", None)
    return cv2.cvtColor(image, code) if code is not None else image[:, :, ::-1]
...
def _device_uses_cuda(device: str, runtime_info: dict[str, Any]) -> bool:
    requested = str(device or "").strip().lower()
    if requested == "cpu":
        return False
    if requested and not (requested == "cuda" or requested.startswith("cuda:") or requested.isdigit()):
        return False
    return bool(runtime_info.get("cuda_available", False))
