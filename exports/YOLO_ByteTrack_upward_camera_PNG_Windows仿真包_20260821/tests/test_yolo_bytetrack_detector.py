...
class YoloByteTrackDetectorTest(unittest.TestCase):
    ...
    def test_selects_highest_confidence_matching_class_with_track_id(self):
        ...
        self.assertFalse(model.calls[0][1]["half"])

    def test_raw_transport_uses_uncompressed_sim_get_images(self):
        tempdir = tempfile.TemporaryDirectory()
        self.addCleanup(tempdir.cleanup)
        model_path = Path(tempdir.name) / "target.pt"
        model_path.write_bytes(b"fake")
        result = SimpleNamespace(boxes=FakeBoxes([], [], [], None))
        ...
        class FakeClient:
            ...
            def simGetImages(self, requests, vehicle_name):
                ...
                payload = np.asarray([255, 0, 0] * 4, dtype=np.uint8).tobytes()
        ...
        self.assertFalse(client.requests[0].compress)
