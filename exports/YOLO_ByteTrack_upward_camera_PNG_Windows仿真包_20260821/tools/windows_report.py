...
def _write_chinese_report(summary: dict[str, Any], results: Sequence[dict[str, Any]], path: Path, plots: Sequence[Path]) -> None:
    ...
    lines.extend(
        [
            ...
            "## 说明",
            "",
            "`fast` 为 Windows 原生 SimpleFlight 快速统计层；`sitl` 为 Windows AirSim 与 WSL2 PX4 v1.11.3 软件在环层。两层控制器和动力学不同，因此分别统计，不直接合并命中率。每个 case 的 CSV、meta、控制台日志和判定 JSON 位于 `cases/`。环境版本记录见 `environment.json`。",
            "",
        ]
    )
