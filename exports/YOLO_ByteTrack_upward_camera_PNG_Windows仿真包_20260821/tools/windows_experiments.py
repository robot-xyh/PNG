...
def _preflight_sitl(config: dict[str, Any]) -> None:
    ...
    text = wslconfig.read_text(encoding="utf-8", errors="replace") if wslconfig.exists() else ""
    if not re.search(r"(?im)^\s*networkingMode\s*=\s*mirrored\s*$", text):
        raise RuntimeError("WSL2 mirrored networking is required. Rerun install_windows.bat; PX4 will not be upgraded.")
    ...
    normalized_listing = listing.stdout.replace("\x00", "")
    distro_line = next((line for line in normalized_listing.splitlines() if distro.lower() in line.lower()), "")
    if listing.returncode != 0 or not re.search(r"\s2\s*$", distro_line):
        raise RuntimeError(f"{distro} must run as WSL2, not WSL1")
    ...
    if vehicle.get("LocalHostIp") != "127.0.0.1" or vehicle.get("ControlIp") != "127.0.0.1":
        raise RuntimeError("PX4/AirSim SITL settings must retain 127.0.0.1 loopback under mirrored networking")
...
def _wait_wsl_px4_ready(distro: str, process: OwnedProcess, timeout_s: float = 45.0) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if completed.returncode == 0:
            return
        time.sleep(0.5)
    raise TimeoutError("Timed out waiting for PX4 SITL TCP 4560 in WSL2")
...
def _start_px4(config: dict[str, Any], session_name: str, log_path: Path, env: dict[str, str]) -> tuple[OwnedProcess, str]:
    ...
    process = _start_owned("PX4 SITL WSL2", command, log_path, cwd=PACKAGE_ROOT, env=env)
    try:
...
def _preflight_runtime(args: argparse.Namespace, config: dict[str, Any], cases: Sequence[CaseSpec]) -> None:
    ...
    if platform.release() != "11" and sys.getwindowsversion().build < 22621:  # type: ignore[attr-defined]
        raise RuntimeError("Windows 11 22H2 or newer is required")
