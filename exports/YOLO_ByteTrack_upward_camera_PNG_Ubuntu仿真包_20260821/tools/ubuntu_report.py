...
def _write_chinese_report(summary: dict[str, Any], results: Sequence[dict[str, Any]], path: Path, plots: Sequence[Path]) -> None:
    lines = [
        ...
        "",
        "场景参数来自 `config/ubuntu_scenarios.json`。`M01-M15` 覆盖距离、侧向、高度差和目标速度组合；`S30-S50` 使用幅值 4 m、周期 8 s 的 S 机动。逐 case 参数及结果见 `cases.csv`。",
        "",
        "## 分层结果",
        "",
    ...
    lines.extend(
        [
            ...
            "## 说明",
            "",
            "`fast` 为 Ubuntu 原生 SimpleFlight 快速统计层；`sitl` 为 Ubuntu 原生 AirSim 与 PX4 v1.11.3 软件在环层。两层控制器和动力学不同，因此分别统计，不直接合并命中率。每个 case 的 CSV、meta、控制台日志和判定 JSON 位于 `cases/`。环境版本记录见 `environment.json`。",
            "",
        ]
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Aggregate Ubuntu AirSim/PX4 case outputs.")
