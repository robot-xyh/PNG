...
try:
    from ubuntu_report import evaluate_case, write_aggregate
except ImportError:  # Imported as tools.ubuntu_experiments by unit tests.
    from tools.ubuntu_report import evaluate_case, write_aggregate


PACKAGE_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CONFIG = PACKAGE_ROOT / "config" / "ubuntu_scenarios.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run repeatable Ubuntu AirSim YOLO+ByteTrack PNG experiments.")
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if int(config.get("schema_version", 0)) != 1:
        raise SystemExit("Unsupported ubuntu_scenarios.json schema_version")
    return config
...
def _port_listeners() -> dict[int, list[str]]:
    completed = subprocess.run(["ss", "-H", "-lntup"], capture_output=True, text=True, errors="replace", check=False)
    if completed.returncode != 0:
        raise RuntimeError(f"ss port inspection failed: {completed.stderr.strip()}")
    listeners: dict[int, list[str]] = {}
    for line in completed.stdout.splitlines():
        for port in PORTS:
            if re.search(rf":{port}\b", line):
                listeners.setdefault(port, []).append(line.strip())
    return listeners


def _assert_ports_free(ports: Sequence[int] = PORTS) -> None:
    listeners = _port_listeners()
    occupied = {port: listeners[port] for port in ports if port in listeners}
    if occupied:
...
def _wait_ports_free(timeout_s: float = 20.0) -> None:
    ...
    while time.monotonic() < deadline:
        listeners = _port_listeners()
        if not any(port in listeners for port in PORTS):
            return
        time.sleep(0.5)
    _assert_ports_free()
...
def _resolve_blocks_executable(config: dict[str, Any]) -> Path:
    ...
    matches = [
        path
        for path in (PACKAGE_ROOT / "runtime" / "Blocks").rglob("Blocks.sh")
        if path.is_file() and os.access(path, os.X_OK)
    ]
    if matches:
        return matches[0]
    raise RuntimeError("Blocks.sh is missing. Run ./install_ubuntu.sh first or set BLOCKS_DIR during installation.")


def _start_blocks(config: dict[str, Any], tier: str, log_path: Path, env: dict[str, str]) -> OwnedProcess:
    _assert_ports_free((41451,))
    executable = _resolve_blocks_executable(config)
...
def _resolve_px4_dir() -> Path:
    marker = PACKAGE_ROOT / "runtime" / "px4_path.txt"
    candidates = []
    if os.environ.get("PX4_DIR"):
        candidates.append(Path(str(os.environ["PX4_DIR"])).expanduser())
    if marker.exists():
        candidates.append(Path(marker.read_text(encoding="utf-8-sig").strip()).expanduser())
    candidates.extend((PACKAGE_ROOT / "runtime" / "PX4-Autopilot", Path.home() / "PX4" / "PX4-Autopilot"))
    for candidate in candidates:
        if (candidate / "Makefile").is_file():
            return candidate.resolve()
    raise RuntimeError("PX4-Autopilot is missing. Run ./install_ubuntu.sh first or set PX4_DIR.")


def _preflight_sitl(config: dict[str, Any]) -> None:
    px4_dir = _resolve_px4_dir()
    ...
    if not (px4_dir / "build" / "px4_sitl_default" / "bin" / "px4").is_file():
        raise RuntimeError(f"PX4 SITL binary is missing under {px4_dir}; rerun ./install_ubuntu.sh")
    settings = json.loads((PACKAGE_ROOT / config["settings"]["sitl"]).read_text(encoding="utf-8"))
    vehicle = settings["Vehicles"]["Interceptor"]
    if vehicle.get("LocalHostIp") != "127.0.0.1" or vehicle.get("ControlIp") != "127.0.0.1":
        raise RuntimeError("PX4/AirSim SITL settings must retain native 127.0.0.1 loopback")


def _wait_px4_ready(process: OwnedProcess, timeout_s: float = 45.0) -> None:
    deadline = time.monotonic() + timeout_s
    while time.monotonic() < deadline:
        if not process.alive():
            raise RuntimeError("PX4 SITL process exited before opening TCP 4560")
        if 4560 in _port_listeners():
            return
        time.sleep(0.5)
    raise TimeoutError("Timed out waiting for native PX4 SITL TCP 4560")


def _start_px4(config: dict[str, Any], log_path: Path, env: dict[str, str]) -> OwnedProcess:
    px4_dir = _resolve_px4_dir()
    px4_env = dict(env)
    px4_env["PX4_SIM_MODEL"] = "iris"
    command = ["make", "px4_sitl_default", "none_iris"]
    process = _start_owned("PX4 SITL", command, log_path, cwd=px4_dir, env=px4_env)
    try:
        _wait_px4_ready(process)
    except Exception:
        process.stop()
        raise
    return process
...
def _environment_manifest(args: argparse.Namespace, config: dict[str, Any], digest: str) -> dict[str, Any]:
    ...
    try:
        ...
        manifest["os_release"] = {
            ...
            "pretty_name": str(os_release.get("PRETTY_NAME", "")),
        }
    except OSError as exc:
        manifest["os_release"] = {"error": str(exc)}
    try:
        import torch
    ...
    manifest["nvidia_smi"] = completed.stdout.strip() if completed.returncode == 0 else "unavailable"
    try:
        blocks = _resolve_blocks_executable(config)
        manifest["blocks_executable"] = str(blocks)
    except RuntimeError:
        manifest["blocks_executable"] = "unavailable"
    if args.tier in {"sitl", "all"}:
        try:
            ...
            manifest["px4_dir"] = str(px4_dir)
            manifest["px4_actual_tag"] = completed.stdout.strip() if completed.returncode == 0 else "unavailable"
        except RuntimeError:
            manifest["px4_dir"] = "unavailable"
            manifest["px4_actual_tag"] = "unavailable"
    return manifest


def _preflight_runtime(args: argparse.Namespace, config: dict[str, Any], cases: Sequence[CaseSpec]) -> None:
    if sys.platform != "linux":
        raise RuntimeError("This launcher requires Ubuntu Linux; use --dry-run only for package inspection elsewhere")
    if platform.machine().lower() not in {"x86_64", "amd64"}:
        raise RuntimeError("Only Ubuntu x86_64 is supported")
    try:
        os_release = platform.freedesktop_os_release()
    except OSError as exc:
        raise RuntimeError(f"Unable to read Ubuntu release information: {exc}") from exc
    if os_release.get("ID") != "ubuntu" or os_release.get("VERSION_ID") not in {"22.04", "24.04"}:
        raise RuntimeError("Ubuntu 22.04 or 24.04 is required")
    os.environ["AIRSIM_RPC_HOST"] = "127.0.0.2"
    os.environ["AIRSIM_REWRITE_HOST_IPS"] = "0"
    if str(config["airsim_rpc_host"]) != "127.0.0.2":
        raise RuntimeError("ubuntu_scenarios.json must use AIRSIM RPC host 127.0.0.2")
    if not MODEL.exists() or MODEL.stat().st_size == 0:
        raise RuntimeError("YOLO model is missing")
    import torch

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA PyTorch is required; fix the NVIDIA driver and rerun ./install_ubuntu.sh")
    _resolve_blocks_executable(config)
    _assert_ports_free()
...
def main() -> None:
            ...
            for retry in range(FAILURE_RETRY_COUNT + 1):
                ...
                try:
                    if spec.tier == "fast":
                        ...
                        _assert_ports_free()
                        px4 = _start_px4(config, attempt_dir / "px4.log", env)
    ...
    print(f"report={output_dir / 'Ubuntu仿真批量测试报告.md'}")
