# YOLO+ByteTrack TTC legacy ClockSpeed0.2 50-100m 复现实验报告
...
恢复旧 50-100m 全命中批次的 LOS KF 默认参数、关闭 LOS 延迟预测，并恢复旧 frame_centering/terminal 速度和横向参数；仅测试 TTC 在 50、60、70、80、90、100m 的可复现性。

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`bytetrack_ttc_legacy_reproduce_50_100_clock0p2_20260624_210014`|
...
![summary](assets/YOLO_ByteTrack_TTC_legacy_reproduce_clock0p2_50_100测试报告/summary_bytetrack_ttc_legacy_reproduce_50_100_clock0p2_20260624_210014.png)

## 4. 汇总表

|组别|命中数|命中距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---|---|---:|---:|---:|---:|
|TTC|2/6|90, 100|50, 60, 70, 80|1.141|819/1466|897/1466|8.07|

## 5. 明细表

|组别|距离m|碰撞|碰撞时间s|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|-|2.820|70.351|54.4%|55.3%|8.09|7.64|1.04|2.51|0.65|
|TTC|60|0|-|1.794|83.534|47.7%|48.1%|8.03|7.58|0.99|1.82|0.42|
|TTC|70|0|-|1.927|87.970|49.7%|54.1%|8.05|7.56|0.99|3.04|0.37|
|TTC|80|0|-|2.008|86.249|44.9%|49.2%|7.87|7.48|0.76|2.69|0.35|
|TTC|90|1|23.85|1.141|1.141|78.0%|87.6%|8.14|7.61|1.30|2.91|0.32|
|TTC|100|1|24.71|1.661|1.661|76.9%|93.5%|8.24|7.68|0.94|2.94|0.29|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![50m](assets/YOLO_ByteTrack_TTC_legacy_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_050m.png)
![60m](assets/YOLO_ByteTrack_TTC_legacy_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_060m.png)
![70m](assets/YOLO_ByteTrack_TTC_legacy_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_070m.png)
![80m](assets/YOLO_ByteTrack_TTC_legacy_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_080m.png)
![90m](assets/YOLO_ByteTrack_TTC_legacy_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_090m.png)
![100m](assets/YOLO_ByteTrack_TTC_legacy_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_100m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|50|2.820|`valid`|no_detection:95, valid:76, area_not_expanding:40, image_kf_predict:9|54.4%|55.3%|
|TTC|60|1.794|`no_detection`|no_detection:129, valid:87, area_not_expanding:30, image_kf_predict:7|47.7%|48.1%|
|TTC|70|1.927|`valid`|no_detection:130, valid:83, area_not_expanding:57, image_kf_predict:17|49.7%|54.1%|
|TTC|80|2.008|`los_innovation_reject`|no_detection:161, valid:94, area_not_expanding:48, image_kf_predict:17|44.9%|49.2%|
|TTC|90|1.141|`no_detection`|valid:90, area_not_expanding:45, no_detection:20, image_kf_predict:19|78.0%|87.6%|
|TTC|100|1.661|`image_kf_predict`|valid:99, area_not_expanding:40, image_kf_predict:32, no_detection:11|76.9%|93.5%|

- LOS KF 参数：`q_lambda=0.0001`、`q_lambda_dot=0.005`、`r=0.005`、`innovation_reject=0.25`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 50m(2.820m)，TTC 60m(1.794m)，TTC 70m(1.927m)，TTC 80m(2.008m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 检测率低于 60% 的工况：TTC 50m(54.4%)，TTC 60m(47.7%)，TTC 70m(49.7%)，TTC 80m(44.9%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 60m:`no_detection`，TTC 80m:`los_innovation_reject`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `1.00 g`，平均需用过载 P95 约 `0.40 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_ByteTrack_TTC_legacy_reproduce_clock0p2_50_100测试报告/shadow_summary_bytetrack_ttc_legacy_reproduce_50_100_clock0p2_20260624_210014.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|2.820|93.3%|1/15|16.2|0.22|2.65|0.65|1.04|
|TTC|60|0|1.794|33.3%|10/15|82.5|0.20|3.44|0.42|0.99|
|TTC|70|0|1.927|86.7%|2/15|47.6|0.20|3.38|0.37|0.99|
|TTC|80|0|2.008|80.0%|3/15|26.7|0.16|2.47|0.35|0.76|
|TTC|90|1|1.141|0.0%|8/8|51.7|0.25|2.03|0.32|1.30|
|TTC|100|1|1.661|75.0%|2/8|21.3|0.24|0.66|0.29|0.94|
...
- TTC: 命中 `2/6`，命中距离 `90m, 100m`，未命中 `50m, 60m, 70m, 80m`，检测帧比例 `55.9%`，有效导引帧比例 `61.2%`，平均检测 FPS `8.07`。
