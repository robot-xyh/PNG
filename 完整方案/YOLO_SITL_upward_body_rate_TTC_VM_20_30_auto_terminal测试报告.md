# YOLO + ByteTrack PX4 SITL upward-camera body-rate TTC / V_m 20-30m auto terminal strategy smoke report
...
Full upward fixed camera smoke test after separating terminal extrapolation defaults: fixed upward acceleration-output runs disable velocity blind-push and use terminal acceleration hold unless explicitly overridden.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_body_rate_yolo_auto_terminal_20260627_225900`|
...
![summary](assets/YOLO_SITL_upward_body_rate_TTC_VM_20_30_auto_terminal测试报告/summary_upward_body_rate_yolo_auto_terminal_20260627_225900.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|0/2|2/2|-|20, 30|-|1.453|165/169|168/169|8.48|
|VM|1/2|1/2|30|20|-|1.464|163/170|167/170|9.21|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|20|0|1|-|11.66|1.453|1.453|1.453|96.3%|98.8%|8.05|7.46|0.76|1.00|0.78|
|VM|20|0|1|-|11.48|1.464|1.464|1.464|94.2%|96.5%|9.18|7.84|0.80|1.07|1.30|
|TTC|30|0|1|-|11.79|1.499|1.499|1.499|98.9%|100.0%|8.92|7.80|0.90|1.48|1.51|
|VM|30|1|0|11.17|-|-|1.656|1.839|97.6%|100.0%|9.25|7.87|0.76|1.87|1.17|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![20m](assets/YOLO_SITL_upward_body_rate_TTC_VM_20_30_auto_terminal测试报告/yolo_sitl_ttc_vm_020m.png)
![30m](assets/YOLO_SITL_upward_body_rate_TTC_VM_20_30_auto_terminal测试报告/yolo_sitl_ttc_vm_030m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|20|1.453|`terminal_lost`|valid:49, area_not_expanding:29, image_kf_predict:2, terminal_lost:2|96.3%|98.8%|
|VM|20|1.464|`terminal_lost`|valid:81, terminal_lost:3, image_kf_predict:2|94.2%|96.5%|
|TTC|30|1.499|`valid`|valid:68, area_not_expanding:17, ttc_out_of_range:1, image_kf_predict:1|98.9%|100.0%|
|VM|30|1.656|`valid`|valid:82, image_kf_predict:1, terminal_lost:1|97.6%|100.0%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 20m(1.453m)，VM 20m(1.464m)，TTC 30m(1.499m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 20m:`terminal_lost`，VM 20m:`terminal_lost`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.80 g`，平均需用过载 P95 约 `1.19 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_SITL_upward_body_rate_TTC_VM_20_30_auto_terminal测试报告/shadow_summary_upward_body_rate_yolo_auto_terminal_20260627_225900.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|20|0|1.453|87.5%|1/8|21.4|0.50|1.55|0.78|0.76|
|VM|20|0|1.464|62.5%|3/8|17.0|0.46|1.61|1.30|0.80|
|TTC|30|0|1.499|100.0%|0/8|29.7|0.46|2.40|1.51|0.90|
|VM|30|1|1.656|88.9%|1/9|43.1|0.44|1.95|1.17|0.76|
...
|---|---:|---:|---:|---|---:|---|---|
|TTC|20|1.453|0.0%|0.0%/0.0%/0.0%|3.7%|90.0/90.0/60.0|0.000/0.000/0.000|
|VM|20|1.464|0.0%|0.0%/0.0%/0.0%|4.7%|90.0/90.0/60.0|0.000/0.000/0.000|
|TTC|30|1.499|0.0%|0.0%/0.0%/0.0%|8.0%|90.0/90.0/60.0|0.000/0.000/0.000|
|VM|30|1.656|0.0%|0.0%/0.0%/0.0%|6.0%|90.0/90.0/60.0|0.000/0.000/0.000|
...
- TTC: 命中 `0/2`，命中距离 `-`，未命中 `20m, 30m`，检测帧比例 `97.6%`，有效导引帧比例 `99.4%`，平均检测 FPS `8.48`。
- VM: 命中 `1/2`，命中距离 `30m`，未命中 `20m`，检测帧比例 `95.9%`，有效导引帧比例 `98.2%`，平均检测 FPS `9.21`。
