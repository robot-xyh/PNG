# YOLO+ByteTrack TTC ClockSpeed0.2 50-100m 复现实验报告
...
使用旧 50-100m 全命中批次的 terminal_capture_lateral_scale=0.08 与 terminal_capture_max_lateral_speed=0.55，重新测试 TTC 在 50、60、70、80、90、100m 的可复现性。

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`bytetrack_ttc_reproduce_50_100_clock0p2_20260624_204255`|
...
![summary](assets/YOLO_ByteTrack_TTC_reproduce_clock0p2_50_100测试报告/summary_bytetrack_ttc_reproduce_50_100_clock0p2_20260624_204255.png)

## 4. 汇总表

|组别|命中数|命中距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---|---|---:|---:|---:|---:|
|TTC|2/6|60, 70|50, 80, 90, 100|1.423|818/1524|943/1524|8.07|

## 5. 明细表

|组别|距离m|碰撞|碰撞时间s|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|-|2.068|50.542|50.4%|54.5%|8.16|7.57|1.12|2.01|1.20|
|TTC|60|1|18.72|1.423|1.715|90.6%|97.8%|8.31|7.66|0.93|3.48|0.43|
|TTC|70|1|24.30|1.560|1.771|77.9%|90.6%|8.16|7.62|0.94|3.33|1.45|
|TTC|80|0|-|1.900|106.058|39.2%|47.5%|7.93|7.51|0.85|2.77|0.40|
|TTC|90|0|-|1.969|93.637|44.0%|53.5%|7.89|7.41|0.83|2.62|0.38|
|TTC|100|0|-|2.038|68.446|50.9%|58.4%|7.98|7.51|1.28|2.60|1.53|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![50m](assets/YOLO_ByteTrack_TTC_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_050m.png)
![60m](assets/YOLO_ByteTrack_TTC_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_060m.png)
![70m](assets/YOLO_ByteTrack_TTC_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_070m.png)
![80m](assets/YOLO_ByteTrack_TTC_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_080m.png)
![90m](assets/YOLO_ByteTrack_TTC_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_090m.png)
![100m](assets/YOLO_ByteTrack_TTC_reproduce_clock0p2_50_100测试报告/yolo_sitl_ttc_vm_100m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|50|2.068|`los_innovation_reject`|no_detection:96, valid:90, image_kf_predict:16, area_not_expanding:14|50.4%|54.5%|
|TTC|60|1.423|`image_kf_predict`|valid:70, area_not_expanding:53, image_kf_predict:10, no_detection:3|90.6%|97.8%|
|TTC|70|1.560|`no_detection`|valid:92, area_not_expanding:46, image_kf_predict:23, no_detection:17|77.9%|90.6%|
|TTC|80|1.900|`image_kf_predict`|no_detection:170, valid:79, area_not_expanding:44, image_kf_predict:27|39.2%|47.5%|
|TTC|90|1.969|`valid`|no_detection:151, valid:85, area_not_expanding:52, image_kf_predict:31|44.0%|53.5%|
|TTC|100|2.038|`bbox_bottom_clipped`|no_detection:135, valid:92, area_not_expanding:66, image_kf_predict:27|50.9%|58.4%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 50m(2.068m)，TTC 80m(1.900m)，TTC 90m(1.969m)，TTC 100m(2.038m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 检测率低于 60% 的工况：TTC 50m(50.4%)，TTC 80m(39.2%)，TTC 90m(44.0%)，TTC 100m(50.9%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 50m:`los_innovation_reject`，TTC 80m:`image_kf_predict`，TTC 100m:`bbox_bottom_clipped`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.99 g`，平均需用过载 P95 约 `0.90 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_ByteTrack_TTC_reproduce_clock0p2_50_100测试报告/shadow_summary_bytetrack_ttc_reproduce_50_100_clock0p2_20260624_204255.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|2.068|26.7%|11/15|62.4|0.24|2.95|1.20|1.12|
|TTC|60|1|1.423|33.3%|6/9|37.2|0.24|2.65|0.43|0.93|
|TTC|70|1|1.560|22.2%|7/9|33.5|0.23|2.29|1.45|0.94|
|TTC|80|0|1.900|40.0%|9/15|78.7|0.15|2.22|0.40|0.85|
|TTC|90|0|1.969|38.5%|8/13|65.8|0.14|2.33|0.38|0.83|
|TTC|100|0|2.038|93.3%|1/15|22.9|0.27|3.19|1.53|1.28|
...
- TTC: 命中 `2/6`，命中距离 `60m, 70m`，未命中 `50m, 80m, 90m, 100m`，检测帧比例 `53.7%`，有效导引帧比例 `61.9%`，平均检测 FPS `8.07`。
