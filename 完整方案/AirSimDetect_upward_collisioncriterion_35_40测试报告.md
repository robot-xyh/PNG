...
|参数|值|
|---|---|
|stamp|`upward_collisioncriterion_detect_20260628_091236`|
...
![summary](assets/AirSimDetect_upward_collisioncriterion_35_40测试报告/summary_upward_collisioncriterion_detect_20260628_091236.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|2/2|2/2|35, 40|35, 40|-|0.734|154/159|157/159|1998.85|
|VM|1/2|1/2|40|40|35|0.496|137/369|139/369|2068.16|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|35|1|1|9.65|9.40|1.429|0.741|0.741|93.6%|100.0%|2010.43|7.98|0.63|1.04|0.54|
|VM|35|0|0|-|-|-|8.975|343.260|20.1%|20.8%|2147.25|7.98|0.79|0.59|0.56|
|TTC|40|1|1|10.02|9.77|1.386|0.734|0.734|100.0%|97.5%|1987.27|7.98|0.58|0.28|0.46|
|VM|40|1|1|10.02|9.77|1.027|0.496|0.875|97.5%|97.5%|1989.08|7.98|0.59|0.85|0.51|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![35m](assets/AirSimDetect_upward_collisioncriterion_35_40测试报告/yolo_sitl_ttc_vm_035m.png)
![40m](assets/AirSimDetect_upward_collisioncriterion_35_40测试报告/yolo_sitl_ttc_vm_040m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|35|0.741|`bbox_bottom_clipped`|valid:56, area_not_expanding:13, image_kf_predict:5, bbox_bottom_clipped:3|93.6%|100.0%|
|VM|35|8.975|`terminal_lost`|no_detection:215, valid:52, terminal_lost:8, image_kf_predict:7|20.1%|20.8%|
|TTC|40|0.734|`bbox_bottom_clipped`|valid:65, area_not_expanding:10, bbox_bottom_clipped:6|100.0%|97.5%|
|VM|40|0.496|`bbox_bottom_clipped`|valid:73, bbox_bottom_clipped:6, image_kf_predict:2|97.5%|97.5%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 检测率低于 60% 的工况：VM 35m(20.1%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：VM 35m:`terminal_lost`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.65 g`，平均需用过载 P95 约 `0.52 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/AirSimDetect_upward_collisioncriterion_35_40测试报告/shadow_summary_upward_collisioncriterion_detect_20260628_091236.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|35|1|0.741|100.0%|0/8|41.8|0.42|0.75|0.54|0.63|
|VM|35|0|8.975|13.3%|13/15|119.8|0.31|0.64|0.56|0.79|
|TTC|40|1|0.734|100.0%|0/8|16.1|0.36|0.57|0.46|0.58|
|VM|40|1|0.496|100.0%|0/9|21.1|0.33|0.52|0.51|0.59|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|35|0.741|94.9%|1.3%|120.0/120.0/42.6|35.0/35.0|0.38/0.85|
|VM|35|8.975|26.7%|4.2%|120.0/120.0/60.0|35.0/35.0|0.25/0.76|
|TTC|40|0.734|100.0%|0.0%|36.0/120.0/37.0|13.0/35.0|0.49/0.77|
|VM|40|0.496|100.0%|1.2%|51.8/120.0/36.9|14.2/35.0|0.25/0.76|
...
- TTC: 命中 `2/2`，命中距离 `35m, 40m`，未命中 `-`，检测帧比例 `96.9%`，有效导引帧比例 `98.7%`，平均检测 FPS `1998.85`。
- VM: 命中 `1/2`，命中距离 `40m`，未命中 `35m`，检测帧比例 `37.1%`，有效导引帧比例 `37.7%`，平均检测 FPS `2068.16`。
