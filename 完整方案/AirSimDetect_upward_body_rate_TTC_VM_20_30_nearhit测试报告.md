# AirSim detect PX4 SITL upward-camera body-rate TTC / V_m 20-30m near-hit smoke report
...
AirSim detect upward fixed camera smoke test: TTC and V_m each run 20m and 30m horizontal range, 30m altitude offset, -10m lateral offset, upward-centering and near-hit diagnostics enabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_body_rate_airsimdetect_nearhit_20260627_223404`|
...
|body-rate tilt / attitude P|`30.0 deg` / `4.0`|
|body-rate roll/pitch max rate|`90.0` / `90.0 deg/s`|
...
|body-rate speed hold|gain `1.2`, max accel `6.0 m/s^2`, total limit `24.0 m/s^2`|
...
|terminal image KF|predict `0.75 s`, reject `0.35 rad`, soft reject `1`|
...
![summary](assets/AirSimDetect_upward_body_rate_TTC_VM_20_30_nearhit测试报告/summary_upward_body_rate_airsimdetect_nearhit_20260627_223404.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|0/2|2/2|-|20, 30|-|1.360|153/175|166/175|2076.66|
|VM|0/2|2/2|-|20, 30|-|1.401|144/169|154/169|2046.63|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|20|0|1|-|10.40|1.419|1.419|1.419|89.3%|100.0%|2094.41|7.98|0.51|1.29|0.64|
|VM|20|0|1|-|10.52|1.401|1.401|1.401|81.2%|87.1%|2114.13|7.98|0.45|1.09|0.69|
|TTC|30|0|1|-|11.27|1.360|1.360|1.360|85.7%|90.1%|2058.91|7.98|0.63|1.10|1.03|
|VM|30|0|1|-|10.40|1.426|1.426|1.426|89.3%|95.2%|1979.14|7.98|0.52|1.10|0.61|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![20m](assets/AirSimDetect_upward_body_rate_TTC_VM_20_30_nearhit测试报告/yolo_sitl_ttc_vm_020m.png)
![30m](assets/AirSimDetect_upward_body_rate_TTC_VM_20_30_nearhit测试报告/yolo_sitl_ttc_vm_030m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|20|1.419|`bbox_area_large`|valid:55, area_not_expanding:14, image_kf_predict:9, bbox_area_large:6|89.3%|100.0%|
|VM|20|1.401|`bbox_area_large`|valid:57, no_detection:8, terminal_lost:8, bbox_area_large:7|81.2%|87.1%|
|TTC|30|1.360|`bbox_area_large`|valid:50, area_not_expanding:16, terminal_lost:8, no_detection:7|85.7%|90.1%|
|VM|30|1.426|`bbox_area_large`|valid:66, terminal_lost:8, image_kf_predict:5, bbox_area_large:3|89.3%|95.2%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 20m(1.419m)，VM 20m(1.401m)，TTC 30m(1.360m)，VM 30m(1.426m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 20m:`bbox_area_large`，VM 20m:`bbox_area_large`，TTC 30m:`bbox_area_large`，VM 30m:`bbox_area_large`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.53 g`，平均需用过载 P95 约 `0.74 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/AirSimDetect_upward_body_rate_TTC_VM_20_30_nearhit测试报告/shadow_summary_upward_body_rate_airsimdetect_nearhit_20260627_223404.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|20|0|1.419|100.0%|0/8|3.4|0.25|0.60|0.64|0.51|
|VM|20|0|1.401|87.5%|1/8|3.8|0.19|0.50|0.69|0.45|
|TTC|30|0|1.360|100.0%|0/8|6.1|0.33|1.05|1.03|0.63|
|VM|30|0|1.426|100.0%|0/8|3.7|0.29|0.54|0.61|0.52|
...
|---|---:|---:|---:|---|---:|---|---|
|TTC|20|1.419|0.0%|0.0%/0.0%/0.0%|4.8%|90.0/90.0/60.0|0.000/0.000/0.000|
|VM|20|1.401|0.0%|0.0%/0.0%/0.0%|2.4%|90.0/90.0/60.0|0.000/0.000/0.000|
|TTC|30|1.360|0.0%|0.0%/0.0%/0.0%|12.1%|90.0/90.0/60.0|0.000/0.000/0.000|
|VM|30|1.426|0.0%|0.0%/0.0%/0.0%|3.6%|90.0/90.0/60.0|0.000/0.000/0.000|
...
- TTC: 命中 `0/2`，命中距离 `-`，未命中 `20m, 30m`，检测帧比例 `87.4%`，有效导引帧比例 `94.9%`，平均检测 FPS `2076.66`。
- VM: 命中 `0/2`，命中距离 `-`，未命中 `20m, 30m`，检测帧比例 `85.2%`，有效导引帧比例 `91.1%`，平均检测 FPS `2046.63`。
