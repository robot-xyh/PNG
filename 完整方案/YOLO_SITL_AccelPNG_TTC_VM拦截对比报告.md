...
|参数|值|
|---|---|
|stamp|`visual_s_maneuver_35_clock0p3_demo_fg_reboot_20260715_200959`|
|settings|`/home/linux/Documents/PNG-px4-upward-camera/.airsim_runtime/settings/airsim_blocks_px4_actor_upward_camera_visual_trace_clock0p3.json`|
...
|body-rate speed hold|gain `1.2`, max accel `6.0 m/s^2`, total limit `28.0 m/s^2`|
...
![summary](assets/YOLO_SITL_AccelPNG_TTC_VM拦截对比报告/summary_visual_s_maneuver_35_clock0p3_demo_fg_reboot_20260715_200959.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/1|1/1|35|35|-|1.272|191/193|193/193|7.76|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|检测FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|35|1|1|8.50|8.41|1.414|1.272|1.329|99.0%|100.0%|7.76|23.24|1.05|1.27|1.21|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、真实 LOS 与 YOLO 检测 LOS 分量、bbox 面积、TTC 估计、实际过载/需用过载和检测 FPS。

![35m](assets/YOLO_SITL_AccelPNG_TTC_VM拦截对比报告/yolo_sitl_ttc_vm_035m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|35|1.272|`image_kf_predict`|valid:145, area_not_expanding:43, ttc_out_of_range:3, image_kf_predict:2|99.0%|100.0%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 本轮平均实际过载峰值约 `1.05 g`，平均需用过载 P95 约 `1.21 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_SITL_AccelPNG_TTC_VM拦截对比报告/shadow_summary_visual_s_maneuver_35_clock0p3_demo_fg_reboot_20260715_200959.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|35|1|1.272|96.0%|1/25|38.6|0.74|1.57|1.21|1.05|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|35|1.272|81.3%|3.6%|60.0/60.0/45.0|35.0/35.0|0.48/0.95|
...
- TTC: 命中 `1/1`，命中距离 `35m`，未命中 `-`，检测帧比例 `99.0%`，有效导引帧比例 `100.0%`，平均检测 FPS `7.76`。
