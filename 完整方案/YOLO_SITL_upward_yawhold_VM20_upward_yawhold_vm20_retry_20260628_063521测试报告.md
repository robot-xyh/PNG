...
|参数|值|
|---|---|
|stamp|`upward_yawhold_vm20_retry_20260628_063521`|
...
![summary](assets/YOLO_SITL_upward_yawhold_VM20_upward_yawhold_vm20_retry_20260628_063521测试报告/summary_upward_yawhold_vm20_retry_20260628_063521.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|VM|1/1|0/1|20|-|-|1.854|78/80|80/80|9.78|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|VM|20|1|0|10.64|-|-|1.854|1.854|97.5%|100.0%|9.78|7.88|0.91|1.97|2.04|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![20m](assets/YOLO_SITL_upward_yawhold_VM20_upward_yawhold_vm20_retry_20260628_063521测试报告/yolo_sitl_ttc_vm_020m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|VM|20|1.854|`terminal_lost`|valid:78, terminal_lost:2|97.5%|100.0%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 本轮平均实际过载峰值约 `0.91 g`，平均需用过载 P95 约 `2.04 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_SITL_upward_yawhold_VM20_upward_yawhold_vm20_retry_20260628_063521测试报告/shadow_summary_upward_yawhold_vm20_retry_20260628_063521.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|VM|20|1|1.854|77.8%|2/9|88.4|0.43|3.85|2.04|0.91|
...
|---|---:|---:|---:|---:|---|---|---|
|VM|20|1.854|60.0%|12.5%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
...
- VM: 命中 `1/1`，命中距离 `20m`，未命中 `-`，检测帧比例 `97.5%`，有效导引帧比例 `100.0%`，平均检测 FPS `9.78`。
