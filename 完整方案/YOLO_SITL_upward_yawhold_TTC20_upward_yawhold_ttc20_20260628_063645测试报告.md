...
|参数|值|
|---|---|
|stamp|`upward_yawhold_ttc20_20260628_063645`|
...
![summary](assets/YOLO_SITL_upward_yawhold_TTC20_upward_yawhold_ttc20_20260628_063645测试报告/summary_upward_yawhold_ttc20_20260628_063645.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/1|0/1|20|-|-|1.734|73/79|74/79|9.46|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|20|1|0|10.52|-|-|1.734|1.734|92.4%|93.7%|9.46|7.88|0.95|0.99|1.02|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![20m](assets/YOLO_SITL_upward_yawhold_TTC20_upward_yawhold_ttc20_20260628_063645测试报告/yolo_sitl_ttc_vm_020m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|20|1.734|`terminal_lost`|valid:46, area_not_expanding:20, terminal_lost:11, image_kf_predict:1|92.4%|93.7%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 本轮平均实际过载峰值约 `0.95 g`，平均需用过载 P95 约 `1.02 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_SITL_upward_yawhold_TTC20_upward_yawhold_ttc20_20260628_063645测试报告/shadow_summary_upward_yawhold_ttc20_20260628_063645.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|20|1|1.734|62.5%|3/8|46.8|0.54|3.16|1.02|0.95|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|20|1.734|74.7%|2.5%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
...
- TTC: 命中 `1/1`，命中距离 `20m`，未命中 `-`，检测帧比例 `92.4%`，有效导引帧比例 `93.7%`，平均检测 FPS `9.46`。
