...
|参数|值|
|---|---|
|stamp|`yolo_async_half_raw_smoke_20260627_003542`|
...
![summary](assets/YOLO_async_half_raw_smoke测试报告/summary_yolo_async_half_raw_smoke_20260627_003542.png)

## 4. 汇总表

|组别|命中数|命中距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---|---|---:|---:|---:|---:|
|TTC|0/1|-|50|35.555|176/364|265/364|26.36|

## 5. 明细表

|组别|距离m|碰撞|碰撞时间s|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|-|35.555|56.190|48.4%|72.8%|26.36|11.94|0.92|2.47|1.53|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![50m](assets/YOLO_async_half_raw_smoke测试报告/yolo_sitl_ttc_vm_050m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|50|35.555|`no_detection`|image_kf_predict:115, area_not_expanding:74, no_detection:62, valid:61|48.4%|72.8%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 检测率低于 60% 的工况：TTC 50m(48.4%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 50m:`no_detection`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.92 g`，平均需用过载 P95 约 `1.53 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_async_half_raw_smoke测试报告/shadow_summary_yolo_async_half_raw_smoke_20260627_003542.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|35.555|39.1%|14/23|86.7|0.10|0.42|1.53|0.92|
...
本轮实际使用 `guidance_output=accel_attitude`、`px4_command_mode=mavlink_attitude`。因此控制链路是“PNG 需用加速度 -> 姿态四元数 + thrust”。
...
- TTC: 命中 `0/1`，命中距离 `-`，未命中 `50m`，检测帧比例 `48.4%`，有效导引帧比例 `72.8%`，平均检测 FPS `26.36`。
