# AirSim Detect upward-camera authority-tuned TTC / V_m 25-40m test report
...
Fixed upward camera detect test after visual-loss-only terminal extrapolation with increased vertical and lateral terminal authority.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_authority_detect_20260628_085017`|
...
|upward centering|`True`, gain `10.0`, max accel `6.0 m/s^2`|
...
|body-rate speed hold|gain `1.2`, max accel `10.0 m/s^2`, total limit `32.0 m/s^2`|
...
![summary](assets/AirSimDetect_upward_authority_25_35_40测试报告/summary_upward_authority_detect_20260628_085017.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|0/3|3/3|-|25, 35, 40|-|1.127|190/207|200/207|1985.17|
|VM|0/3|3/3|-|25, 35, 40|-|1.019|189/199|199/199|2003.45|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|25|0|1|-|7.77|1.468|1.468|1.468|88.9%|100.0%|1994.48|7.98|0.66|0.89|0.99|
|VM|25|0|1|-|7.64|1.194|1.194|1.194|91.9%|100.0%|1953.21|7.98|0.68|0.77|0.78|
|TTC|35|0|1|-|8.52|1.197|1.197|1.197|98.6%|100.0%|1994.86|7.98|0.68|0.94|0.61|
|VM|35|0|1|-|8.27|1.224|1.224|1.224|95.5%|100.0%|2021.59|7.99|0.65|0.55|0.61|
|TTC|40|0|1|-|9.27|1.127|1.127|1.127|88.0%|90.7%|1966.16|7.98|0.66|1.23|0.65|
|VM|40|0|1|-|8.64|1.019|1.019|1.019|97.1%|100.0%|2035.55|7.98|0.69|0.73|0.58|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![25m](assets/AirSimDetect_upward_authority_25_35_40测试报告/yolo_sitl_ttc_vm_025m.png)
![35m](assets/AirSimDetect_upward_authority_25_35_40测试报告/yolo_sitl_ttc_vm_035m.png)
![40m](assets/AirSimDetect_upward_authority_25_35_40测试报告/yolo_sitl_ttc_vm_040m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|25|1.468|`bbox_bottom_clipped`|valid:46, area_not_expanding:9, image_kf_predict:7, bbox_bottom_clipped:1|88.9%|100.0%|
|VM|25|1.194|`valid`|valid:57, image_kf_predict:5|91.9%|100.0%|
|TTC|35|1.197|`bbox_bottom_clipped`|valid:52, area_not_expanding:14, bbox_bottom_clipped:2, image_kf_predict:1|98.6%|100.0%|
|VM|35|1.224|`valid`|valid:64, image_kf_predict:3|95.5%|100.0%|
|TTC|40|1.127|`bbox_bottom_clipped`|valid:51, area_not_expanding:12, no_detection:6, bbox_bottom_clipped:3|88.0%|90.7%|
|VM|40|1.019|`valid`|valid:68, image_kf_predict:2|97.1%|100.0%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 25m(1.468m)，VM 25m(1.194m)，TTC 35m(1.197m)，VM 35m(1.224m)，TTC 40m(1.127m)，VM 40m(1.019m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 25m:`bbox_bottom_clipped`，TTC 35m:`bbox_bottom_clipped`，TTC 40m:`bbox_bottom_clipped`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.67 g`，平均需用过载 P95 约 `0.70 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/AirSimDetect_upward_authority_25_35_40测试报告/shadow_summary_upward_authority_detect_20260628_085017.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|25|0|1.468|100.0%|0/8|5.8|0.69|1.34|0.99|0.66|
|VM|25|0|1.194|100.0%|0/8|2.7|0.42|0.72|0.78|0.68|
|TTC|35|0|1.197|100.0%|0/8|6.8|0.58|1.09|0.61|0.68|
|VM|35|0|1.224|100.0%|0/8|3.0|0.42|0.66|0.61|0.65|
|TTC|40|0|1.127|87.5%|1/8|5.4|0.50|0.89|0.65|0.66|
|VM|40|0|1.019|87.5%|1/8|5.8|0.42|0.59|0.58|0.69|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|25|1.468|41.3%|3.2%|120.0/120.0/60.0|35.0/35.0|0.38/0.85|
|VM|25|1.194|45.2%|6.5%|120.0/120.0/47.5|35.0/35.0|0.45/0.85|
|TTC|35|1.197|73.9%|1.4%|60.7/120.0/34.0|28.8/35.0|0.63/0.85|
|VM|35|1.224|77.6%|1.5%|88.8/120.0/34.7|19.6/35.0|0.67/0.85|
|TTC|40|1.127|88.0%|2.7%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
|VM|40|1.019|87.1%|1.4%|44.4/120.0/36.7|13.9/35.0|0.65/0.85|
...
- TTC: 命中 `0/3`，命中距离 `-`，未命中 `25m, 35m, 40m`，检测帧比例 `91.8%`，有效导引帧比例 `96.6%`，平均检测 FPS `1985.17`。
- VM: 命中 `0/3`，命中距离 `-`，未命中 `25m, 35m, 40m`，检测帧比例 `95.0%`，有效导引帧比例 `100.0%`，平均检测 FPS `2003.45`。
