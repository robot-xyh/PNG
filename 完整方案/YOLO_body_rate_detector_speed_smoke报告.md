# YOLO detector speed smoke under mavlink body-rate baseline
...
|参数|值|
|---|---|
|stamp|`bodyrate_async_compressed_half_20260627_023605`|
...
![summary](assets/YOLO_body_rate_detector_speed_smoke报告/summary_bodyrate_async_compressed_half_20260627_023605.png)

## 4. 汇总表

|组别|命中数|命中距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---|---|---:|---:|---:|---:|
|TTC|1/1|50|-|1.734|87/97|89/97|10.32|

## 5. 明细表

|组别|距离m|碰撞|碰撞时间s|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|1|12.04|1.734|1.734|89.7%|91.8%|10.32|7.97|0.61|1.21|0.52|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![50m](assets/YOLO_body_rate_detector_speed_smoke报告/yolo_sitl_ttc_vm_050m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|50|1.734|`image_kf_predict`|valid:69, area_not_expanding:16, async_detection_waiting:8, image_kf_predict:2|89.7%|91.8%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=0.75`。
- 本轮平均实际过载峰值约 `0.61 g`，平均需用过载 P95 约 `0.52 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_body_rate_detector_speed_smoke报告/shadow_summary_bodyrate_async_compressed_half_20260627_023605.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|1|1.734|87.5%|1/8|19.7|0.38|1.92|0.52|0.61|
...
|---|---:|---:|---:|---|---:|---|---|
|TTC|50|1.734|0.0%|0.0%/0.0%/0.0%|3.1%|60.0/60.0/45.0|0.000/0.000/0.000|
...
- TTC: 命中 `1/1`，命中距离 `50m`，未命中 `-`，检测帧比例 `89.7%`，有效导引帧比例 `91.8%`，平均检测 FPS `10.32`。
