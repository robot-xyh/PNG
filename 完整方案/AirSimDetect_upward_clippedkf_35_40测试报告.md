...
|参数|值|
|---|---|
|stamp|`upward_clippedkf_detect_20260628_090430`|
...
![summary](assets/AirSimDetect_upward_clippedkf_35_40测试报告/summary_upward_clippedkf_detect_20260628_090430.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|0/2|2/2|-|35, 40|-|0.938|154/156|154/156|2002.19|
|VM|0/2|2/2|-|35, 40|-|1.220|149/152|150/152|2039.73|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|35|0|1|-|9.40|1.281|1.281|1.281|97.4%|100.0%|2011.53|7.98|0.54|0.43|0.50|
|VM|35|0|1|-|9.15|1.220|1.220|1.220|100.0%|100.0%|1994.42|7.98|0.59|0.32|0.47|
|TTC|40|0|1|-|9.90|0.938|0.938|0.938|100.0%|97.5%|1992.85|7.98|0.64|0.33|0.47|
|VM|40|0|1|-|9.65|1.448|1.448|1.448|96.2%|97.4%|2085.04|7.98|0.62|0.75|0.51|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![35m](assets/AirSimDetect_upward_clippedkf_35_40测试报告/yolo_sitl_ttc_vm_035m.png)
![40m](assets/AirSimDetect_upward_clippedkf_35_40测试报告/yolo_sitl_ttc_vm_040m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|35|1.281|`bbox_bottom_clipped`|valid:59, area_not_expanding:14, image_kf_predict:2, bbox_bottom_clipped:1|97.4%|100.0%|
|VM|35|1.220|`bbox_bottom_clipped`|valid:73, bbox_bottom_clipped:1|100.0%|100.0%|
|TTC|40|0.938|`bbox_bottom_clipped`|valid:65, area_not_expanding:10, bbox_bottom_clipped:5|100.0%|97.5%|
|VM|40|1.448|`bbox_bottom_clipped`|valid:72, bbox_bottom_clipped:3, image_kf_predict:3|96.2%|97.4%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 35m(1.281m)，VM 35m(1.220m)，TTC 40m(0.938m)，VM 40m(1.448m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 35m:`bbox_bottom_clipped`，VM 35m:`bbox_bottom_clipped`，TTC 40m:`bbox_bottom_clipped`，VM 40m:`bbox_bottom_clipped`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.60 g`，平均需用过载 P95 约 `0.49 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/AirSimDetect_upward_clippedkf_35_40测试报告/shadow_summary_upward_clippedkf_detect_20260628_090430.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|35|0|1.281|100.0%|0/8|8.1|0.38|0.62|0.50|0.54|
|VM|35|0|1.220|100.0%|0/8|5.3|0.33|0.51|0.47|0.59|
|TTC|40|0|0.938|100.0%|0/8|4.9|0.36|0.56|0.47|0.64|
|VM|40|0|1.448|100.0%|0/8|4.1|0.31|0.47|0.51|0.62|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|35|1.281|94.7%|1.3%|120.0/120.0/34.3|35.0/33.9|0.54/0.85|
|VM|35|1.220|94.6%|0.0%|116.1/120.0/36.0|28.6/33.9|0.60/0.83|
|TTC|40|0.938|98.8%|0.0%|35.8/120.0/36.9|13.0/35.0|0.49/0.77|
|VM|40|1.448|97.4%|1.3%|120.0/120.0/37.1|35.0/35.0|0.49/0.85|
...
- TTC: 命中 `0/2`，命中距离 `-`，未命中 `35m, 40m`，检测帧比例 `98.7%`，有效导引帧比例 `98.7%`，平均检测 FPS `2002.19`。
- VM: 命中 `0/2`，命中距离 `-`，未命中 `35m, 40m`，检测帧比例 `98.0%`，有效导引帧比例 `98.7%`，平均检测 FPS `2039.73`。
