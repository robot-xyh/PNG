...
|参数|值|
|---|---|
|stamp|`upward_detect_more_20260628_065534`|
...
![summary](assets/AirSimDetect_upward_yawhold_more_upward_detect_more_20260628_065534测试报告/summary_upward_detect_more_20260628_065534.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|0/3|3/3|-|25, 35, 40|-|1.176|220/231|230/231|2072.89|
|VM|0/3|3/3|-|25, 35, 40|-|1.084|216/224|224/224|2083.05|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|25|0|1|-|9.52|1.176|1.176|1.176|89.6%|98.7%|2096.23|7.98|0.71|0.98|0.67|
|VM|25|0|1|-|9.14|1.454|1.454|1.454|90.5%|100.0%|2118.02|7.98|0.64|0.89|0.89|
|TTC|35|0|1|-|9.40|1.233|1.233|1.233|98.7%|100.0%|2036.95|7.98|0.61|0.40|0.46|
|VM|35|0|1|-|9.02|1.379|1.379|1.379|98.6%|100.0%|2024.28|7.98|0.64|0.44|0.48|
|TTC|40|0|1|-|9.65|1.353|1.353|1.353|97.4%|100.0%|2085.48|7.98|0.60|0.28|0.48|
|VM|40|0|1|-|9.52|1.084|1.084|1.084|100.0%|100.0%|2106.84|7.98|0.54|0.74|0.47|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![25m](assets/AirSimDetect_upward_yawhold_more_upward_detect_more_20260628_065534测试报告/yolo_sitl_ttc_vm_025m.png)
![35m](assets/AirSimDetect_upward_yawhold_more_upward_detect_more_20260628_065534测试报告/yolo_sitl_ttc_vm_035m.png)
![40m](assets/AirSimDetect_upward_yawhold_more_upward_detect_more_20260628_065534测试报告/yolo_sitl_ttc_vm_040m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|25|1.176|`terminal_lost`|valid:51, area_not_expanding:11, image_kf_predict:7, terminal_lost:7|89.6%|98.7%|
|VM|25|1.454|`bbox_area_large`|valid:53, terminal_lost:8, bbox_area_large:8, image_kf_predict:5|90.5%|100.0%|
|TTC|35|1.233|`bbox_area_large`|valid:55, area_not_expanding:14, bbox_area_large:6, image_kf_predict:1|98.7%|100.0%|
|VM|35|1.379|`valid`|valid:65, terminal_lost:8|98.6%|100.0%|
|TTC|40|1.353|`bbox_bottom_clipped`|bbox_bottom_clipped:78|97.4%|100.0%|
|VM|40|1.084|`bbox_area_large`|valid:69, bbox_area_large:8|100.0%|100.0%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 25m(1.176m)，VM 25m(1.454m)，TTC 35m(1.233m)，VM 35m(1.379m)，TTC 40m(1.353m)，VM 40m(1.084m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 25m:`terminal_lost`，VM 25m:`bbox_area_large`，TTC 35m:`bbox_area_large`，TTC 40m:`bbox_bottom_clipped`，VM 40m:`bbox_area_large`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.62 g`，平均需用过载 P95 约 `0.57 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/AirSimDetect_upward_yawhold_more_upward_detect_more_20260628_065534测试报告/shadow_summary_upward_detect_more_20260628_065534.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|25|0|1.176|87.5%|1/8|6.9|0.59|1.62|0.67|0.71|
|VM|25|0|1.454|100.0%|0/8|9.6|0.44|1.52|0.89|0.64|
|TTC|35|0|1.233|100.0%|0/8|5.3|0.38|0.62|0.46|0.61|
|VM|35|0|1.379|100.0%|0/8|12.1|0.35|0.63|0.48|0.64|
|TTC|40|0|1.353|100.0%|0/8|1.7|0.38|0.59|0.48|0.60|
|VM|40|0|1.084|100.0%|0/8|17.3|0.32|0.47|0.47|0.54|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|25|1.176|31.2%|1.3%|120.0/120.0/60.0|35.0/35.0|0.25/0.83|
|VM|25|1.454|32.4%|1.4%|120.0/120.0/60.0|35.0/35.0|0.25/0.82|
|TTC|35|1.233|94.7%|1.3%|38.3/120.0/34.0|14.4/33.9|0.63/0.85|
|VM|35|1.379|94.5%|0.0%|120.0/120.0/35.5|35.0/33.9|0.57/0.76|
|TTC|40|1.353|97.4%|0.0%|35.8/120.0/37.3|12.5/35.0|0.61/0.76|
|VM|40|1.084|98.7%|0.0%|35.4/120.0/36.6|11.5/35.0|0.64/0.74|
...
- TTC: 命中 `0/3`，命中距离 `-`，未命中 `25m, 35m, 40m`，检测帧比例 `95.2%`，有效导引帧比例 `99.6%`，平均检测 FPS `2072.89`。
- VM: 命中 `0/3`，命中距离 `-`，未命中 `25m, 35m, 40m`，检测帧比例 `96.4%`，有效导引帧比例 `100.0%`，平均检测 FPS `2083.05`。
