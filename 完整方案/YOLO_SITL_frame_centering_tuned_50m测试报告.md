# YOLO + ByteTrack PX4 SITL frame_centering tuned 50m 测试报告
...
只测试 50m，验证放宽 frame_centering / terminal_capture 速度限制后的效果。

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`frame_centering_tuned_50m_20260621_054436`|
...
![summary](assets/YOLO_SITL_frame_centering_tuned_50m测试报告/summary_frame_centering_tuned_50m_20260621_054436.png)

## 4. 汇总表

|组别|命中数|命中距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---|---|---:|---:|---:|---:|
|TTC|1/1|50|-|1.800|114/124|120/124|8.20|
|VM|0/1|-|50|2.063|193/226|162/226|8.15|

## 5. 明细表

|组别|距离m|碰撞|碰撞时间s|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|指令P95 g|导引评估P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|1|17.10|1.800|1.800|91.9%|96.8%|8.20|7.57|0.98|3.34|0.51|
|VM|50|0|-|2.063|11.140|85.4%|71.7%|8.15|7.60|1.02|2.45|0.80|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/导引评估过载和 YOLO 检测 FPS。

![50m](assets/YOLO_SITL_frame_centering_tuned_50m测试报告/yolo_sitl_ttc_vm_050m.png)

## 7. 结论

- TTC: 命中 `1/1`，命中距离 `50m`，未命中 `-`，检测帧比例 `91.9%`，有效导引帧比例 `96.8%`，平均检测 FPS `8.20`。
- VM: 命中 `0/1`，命中距离 `-`，未命中 `50m`，检测帧比例 `85.4%`，有效导引帧比例 `71.7%`，平均检测 FPS `8.15`。
