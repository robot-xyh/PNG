# YOLO + ByteTrack PX4 SITL upward-camera body-rate TTC 30m terminal accel-hold retest
...
Focused retest of TTC 30m with upward fixed camera terminal strategy: velocity blind-push disabled and terminal acceleration hold enabled.

## 2. 基准条件

|参数|值|
|---|---|
|stamp|`upward_body_rate_yolo_ttc30_terminal_accel_20260627_225218`|
...
![summary](assets/YOLO_SITL_upward_body_rate_TTC_30_terminal_accel测试报告/summary_upward_body_rate_yolo_ttc30_terminal_accel_20260627_225218.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|0/1|1/1|-|30|-|1.453|85/89|89/89|9.06|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|30|0|1|-|11.89|1.453|1.453|1.453|95.5%|100.0%|9.06|7.84|0.92|1.22|0.98|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![30m](assets/YOLO_SITL_upward_body_rate_TTC_30_terminal_accel测试报告/yolo_sitl_ttc_vm_030m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|30|1.453|`terminal_lost`|valid:76, area_not_expanding:8, terminal_lost:5|95.5%|100.0%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 30m(1.453m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 30m:`terminal_lost`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.92 g`，平均需用过载 P95 约 `0.98 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_SITL_upward_body_rate_TTC_30_terminal_accel测试报告/shadow_summary_upward_body_rate_yolo_ttc30_terminal_accel_20260627_225218.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|30|0|1.453|50.0%|4/8|27.4|0.52|3.11|0.98|0.92|
...
|---|---:|---:|---:|---|---:|---|---|
|TTC|30|1.453|0.0%|0.0%/0.0%/0.0%|5.6%|90.0/90.0/60.0|0.000/0.000/0.000|
...
- TTC: 命中 `0/1`，命中距离 `-`，未命中 `30m`，检测帧比例 `95.5%`，有效导引帧比例 `100.0%`，平均检测 FPS `9.06`。
