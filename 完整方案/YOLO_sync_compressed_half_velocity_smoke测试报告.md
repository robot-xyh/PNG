...
|参数|值|
|---|---|
|stamp|`yolo_sync_compressed_half_velocity_smoke_20260627_004809`|
...
![summary](assets/YOLO_sync_compressed_half_velocity_smoke测试报告/summary_yolo_sync_compressed_half_velocity_smoke_20260627_004809.png)

## 4. 汇总表

|组别|命中数|命中距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---|---|---:|---:|---:|---:|
|TTC|0/1|-|50|2.004|107/220|115/220|7.91|

## 5. 明细表

|组别|距离m|碰撞|碰撞时间s|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|-|2.004|75.013|48.6%|52.3%|7.91|7.45|1.11|1.93|0.36|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![50m](assets/YOLO_sync_compressed_half_velocity_smoke测试报告/yolo_sitl_ttc_vm_050m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|50|2.004|`no_detection`|no_detection:105, valid:83, area_not_expanding:18, image_kf_predict:8|48.6%|52.3%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 50m(2.004m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 检测率低于 60% 的工况：TTC 50m(48.6%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 50m:`no_detection`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `1.11 g`，平均需用过载 P95 约 `0.36 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_sync_compressed_half_velocity_smoke测试报告/shadow_summary_yolo_sync_compressed_half_velocity_smoke_20260627_004809.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|2.004|0.0%|15/15|160.2|0.24|3.27|0.36|1.11|
...
本轮不是 `accel_body_rate + body-rate-control-profile=v2`，不生成 body-rate v2 专属诊断。

## 10. PNG 到过载、姿态和角速度的控制流程

本轮实际使用 `guidance_output=velocity_bias`、`px4_command_mode=velocity_yaw_rate`。因此控制链路保留为速度指令/航向率指令形式。
...
- TTC: 命中 `0/1`，命中距离 `-`，未命中 `50m`，检测帧比例 `48.6%`，有效导引帧比例 `52.3%`，平均检测 FPS `7.91`。
