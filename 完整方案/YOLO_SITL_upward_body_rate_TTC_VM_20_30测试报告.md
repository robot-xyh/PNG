...
|参数|值|
|---|---|
|stamp|`upward_body_rate_yolo_nearhit_20260627_223759`|
...
![summary](assets/YOLO_SITL_upward_body_rate_TTC_VM_20_30测试报告/summary_upward_body_rate_yolo_nearhit_20260627_223759.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|0/2|1/2|-|20|30|1.297|289/384|280/384|9.21|
|VM|0/2|2/2|-|20, 30|-|1.322|167/171|170/171|9.65|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|20|0|1|-|13.68|1.297|1.297|1.297|92.3%|82.7%|9.28|7.88|0.85|1.00|0.87|
|VM|20|0|1|-|11.23|1.455|1.455|1.455|98.8%|100.0%|9.65|7.88|0.70|1.01|1.09|
|TTC|30|0|0|-|-|-|1.607|61.046|68.9%|69.3%|9.15|7.90|0.95|1.77|1.87|
|VM|30|0|1|-|11.30|1.322|1.322|1.322|96.5%|98.8%|9.66|7.89|0.70|1.01|0.94|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![20m](assets/YOLO_SITL_upward_body_rate_TTC_VM_20_30测试报告/yolo_sitl_ttc_vm_020m.png)
![30m](assets/YOLO_SITL_upward_body_rate_TTC_VM_20_30测试报告/yolo_sitl_ttc_vm_030m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|20|1.297|`los_innovation_reject`|valid:58, area_not_expanding:17, terminal_lost:16, los_innovation_reject:12|92.3%|82.7%|
|VM|20|1.455|`valid`|valid:77, terminal_lost:8|98.8%|100.0%|
|TTC|30|1.607|`terminal_lost`|valid:101, no_detection:63, terminal_lost:56, area_not_expanding:54|68.9%|69.3%|
|VM|30|1.322|`terminal_lost`|valid:75, terminal_lost:11|96.5%|98.8%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 20m(1.297m)，VM 20m(1.455m)，TTC 30m(1.607m)，VM 30m(1.322m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 20m:`los_innovation_reject`，TTC 30m:`terminal_lost`，VM 30m:`terminal_lost`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.80 g`，平均需用过载 P95 约 `1.19 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_SITL_upward_body_rate_TTC_VM_20_30测试报告/shadow_summary_upward_body_rate_yolo_nearhit_20260627_223759.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|20|0|1.297|100.0%|0/8|26.8|0.52|3.68|0.87|0.85|
|VM|20|0|1.455|100.0%|0/8|33.0|0.30|1.02|1.09|0.70|
|TTC|30|0|1.607|80.0%|3/15|111.4|0.38|2.88|1.87|0.95|
|VM|30|0|1.322|75.0%|2/8|50.4|0.46|1.80|0.94|0.70|
...
|---|---:|---:|---:|---|---:|---|---|
|TTC|20|1.297|0.0%|0.0%/0.0%/0.0%|6.7%|90.0/90.0/60.0|0.000/0.000/0.000|
|VM|20|1.455|0.0%|0.0%/0.0%/0.0%|3.5%|90.0/90.0/60.0|0.000/0.000/0.000|
|TTC|30|1.607|0.0%|0.0%/0.0%/0.0%|19.3%|90.0/90.0/60.0|0.000/0.000/0.000|
|VM|30|1.322|0.0%|0.0%/0.0%/0.0%|2.3%|90.0/90.0/60.0|0.000/0.000/0.000|
...
- TTC: 命中 `0/2`，命中距离 `-`，未命中 `20m, 30m`，检测帧比例 `75.3%`，有效导引帧比例 `72.9%`，平均检测 FPS `9.21`。
- VM: 命中 `0/2`，命中距离 `-`，未命中 `20m, 30m`，检测帧比例 `97.7%`，有效导引帧比例 `99.4%`，平均检测 FPS `9.65`。
