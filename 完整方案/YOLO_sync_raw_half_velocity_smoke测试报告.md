...
|参数|值|
|---|---|
|stamp|`yolo_sync_raw_half_velocity_smoke_20260627_004344`|
...
![summary](assets/YOLO_sync_raw_half_velocity_smoke测试报告/summary_yolo_sync_raw_half_velocity_smoke_20260627_004344.png)

## 4. 汇总表

|组别|命中数|命中距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---|---|---:|---:|---:|---:|
|TTC|0/1|-|50|45.725|67/237|126/237|16.73|

## 5. 明细表

|组别|距离m|碰撞|碰撞时间s|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|-|45.725|135.862|28.3%|53.2%|16.73|7.94|1.03|3.47|1.53|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![50m](assets/YOLO_sync_raw_half_velocity_smoke测试报告/yolo_sitl_ttc_vm_050m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|50|45.725|`valid`|no_detection:107, image_kf_predict:61, valid:29, area_not_expanding:19|28.3%|53.2%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 检测率低于 60% 的工况：TTC 50m(28.3%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 本轮平均实际过载峰值约 `1.03 g`，平均需用过载 P95 约 `1.53 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_sync_raw_half_velocity_smoke测试报告/shadow_summary_yolo_sync_raw_half_velocity_smoke_20260627_004344.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|50|0|45.725|53.3%|7/15|104.3|0.06|0.38|1.53|1.03|
...
- TTC: 命中 `0/1`，命中距离 `-`，未命中 `50m`，检测帧比例 `28.3%`，有效导引帧比例 `53.2%`，平均检测 FPS `16.73`。
