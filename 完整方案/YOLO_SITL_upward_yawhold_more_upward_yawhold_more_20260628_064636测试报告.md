...
|参数|值|
|---|---|
|stamp|`upward_yawhold_more_20260628_064636`|
...
![summary](assets/YOLO_SITL_upward_yawhold_more_upward_yawhold_more_20260628_064636测试报告/summary_upward_yawhold_more_20260628_064636.png)

## 4. 汇总表

|组别|碰撞命中|近距命中|碰撞距离m|近距距离m|未命中距离m|最小中心距离m|检测帧/总帧|有效帧/总帧|平均检测FPS|
|---|---:|---:|---|---|---|---:|---:|---:|---:|
|TTC|1/3|0/3|40|-|25, 35|1.864|235/639|229/639|9.24|
|VM|1/3|0/3|25|-|35, 40|1.628|216/636|225/636|9.38|

## 5. 明细表

|组别|距离m|碰撞|近距|碰撞时间s|近距时间s|近距距离m|最小距离m|终点距离m|检测帧率|有效帧率|YOLO FPS|sim FPS|实际过载max g|速度指令差分P95 g|需用过载P95 g|
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|25|0|0|-|-|-|2.196|114.481|30.5%|27.2%|9.00|7.87|0.87|0.46|0.59|
|VM|25|1|0|9.91|-|-|1.628|1.666|95.9%|97.3%|9.72|7.89|0.80|0.82|0.66|
|TTC|35|0|0|-|-|-|3.427|269.780|25.2%|26.3%|8.94|7.87|0.85|0.63|0.52|
|VM|35|0|0|-|-|-|2.071|163.174|26.0%|27.0%|9.24|7.92|0.81|0.78|0.56|
|TTC|40|1|0|10.83|-|-|1.864|1.864|97.6%|97.6%|9.79|7.89|0.86|1.13|0.75|
|VM|40|0|0|-|-|-|3.065|316.499|25.6%|27.4%|9.19|7.93|0.88|0.68|0.63|

## 6. 分距离曲线

每个距离一张图，包含真实中心距离、bbox 面积、TTC 估计、实际过载/需用过载和 YOLO 检测 FPS。

![25m](assets/YOLO_SITL_upward_yawhold_more_upward_yawhold_more_20260628_064636测试报告/yolo_sitl_ttc_vm_025m.png)
![35m](assets/YOLO_SITL_upward_yawhold_more_upward_yawhold_more_20260628_064636测试报告/yolo_sitl_ttc_vm_035m.png)
![40m](assets/YOLO_SITL_upward_yawhold_more_upward_yawhold_more_20260628_064636测试报告/yolo_sitl_ttc_vm_040m.png)

## 7. LOS KF 与失败原因诊断

|组别|距离m|最近距离m|最近点状态|主要失败/降级原因|检测率|有效率|
|---|---:|---:|---|---|---:|---:|
|TTC|25|2.196|`terminal_lost`|no_detection:187, valid:51, terminal_lost:17, area_not_expanding:15|30.5%|27.2%|
|VM|25|1.628|`terminal_lost`|valid:71, terminal_lost:2, image_kf_predict:1|95.9%|97.3%|
|TTC|35|3.427|`terminal_lost`|no_detection:196, valid:50, terminal_lost:16, area_not_expanding:15|25.2%|26.3%|
|VM|35|2.071|`no_detection`|no_detection:205, valid:67, terminal_lost:8, image_kf_predict:1|26.0%|27.0%|
|TTC|40|1.864|`bbox_bottom_clipped`|bbox_bottom_clipped:82|97.6%|97.6%|
|VM|40|3.065|`terminal_lost`|no_detection:195, valid:70, terminal_lost:16|25.6%|27.4%|

- LOS KF 参数：`q_lambda=0.0005`、`q_lambda_dot=0.02`、`r=0.008`、`innovation_reject=0.75`、`terminal_reject=1.2`。
- 未命中但最近距离小于等于 3m 的工况：TTC 25m(2.196m)，VM 35m(2.071m)。这些工况已接近目标，但没有触发 AirSim 碰撞判定，后续应重点看末端视场保持、外推和碰撞几何。
- 检测率低于 60% 的工况：TTC 25m(30.5%)，TTC 35m(25.2%)，VM 35m(26.0%)，VM 40m(25.6%)。这类失败优先归因于 YOLO/ByteTrack 连续性和固定相机视场保持，而不是导引律公式本身。
- 最近点处仍处于降级或无效状态的未命中工况：TTC 25m:`terminal_lost`，TTC 35m:`terminal_lost`，VM 35m:`no_detection`，VM 40m:`terminal_lost`。这些样本说明末端质量门、视觉外推和 bbox 裁切处理仍会影响命中窗口。
- 本轮平均实际过载峰值约 `0.85 g`，平均需用过载 P95 约 `0.62 g`。两者不是同一个量：`n_cmd_g` 是导引层需求，实际过载还受 PX4 姿态/推力限制、YOLO 约 9 FPS 采样和 frame centering 限速影响。

## 8. 相机光心真值影子测试诊断

![shadow_summary](assets/YOLO_SITL_upward_yawhold_more_upward_yawhold_more_20260628_064636测试报告/shadow_summary_upward_yawhold_more_20260628_064636.png)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
|TTC|25|0|2.196|66.7%|5/15|96.0|0.22|2.28|0.59|0.87|
|VM|25|1|1.628|77.8%|2/9|49.6|0.37|1.64|0.66|0.80|
|TTC|35|0|3.427|20.0%|12/15|117.6|0.28|0.69|0.52|0.85|
|VM|35|0|2.071|33.3%|10/15|92.9|0.27|2.14|0.56|0.81|
|TTC|40|1|1.864|87.5%|1/8|29.2|0.60|2.08|0.75|0.86|
|VM|40|0|3.065|13.3%|13/15|146.8|0.24|0.43|0.63|0.88|
...
|---|---:|---:|---:|---:|---|---|---|
|TTC|25|2.196|31.2%|2.2%|120.0/120.0/60.0|35.0/35.0|0.25/0.84|
|VM|25|1.628|85.1%|1.4%|120.0/120.0/60.0|35.0/35.0|0.62/0.85|
|TTC|35|3.427|28.8%|3.6%|120.0/120.0/60.0|35.0/35.0|0.25/0.82|
|VM|35|2.071|29.5%|1.1%|120.0/120.0/60.0|35.0/33.8|0.25/0.83|
|TTC|40|1.864|100.0%|3.7%|120.0/120.0/60.0|35.0/35.0|0.25/0.82|
|VM|40|3.065|28.8%|3.9%|120.0/120.0/60.0|35.0/35.0|0.25/0.85|
...
- TTC: 命中 `1/3`，命中距离 `40m`，未命中 `25m, 35m`，检测帧比例 `36.8%`，有效导引帧比例 `35.8%`，平均检测 FPS `9.24`。
- VM: 命中 `1/3`，命中距离 `25m`，未命中 `35m, 40m`，检测帧比例 `34.0%`，有效导引帧比例 `35.4%`，平均检测 FPS `9.38`。
