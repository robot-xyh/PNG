...
- 相机输出分辨率和本机固定安装外参已确认；内参标定归档、真实曝光时间戳和飞行振动下的外参
  保持性仍未确认。
...
无桨限值、相机外参和RAW_IMU轴符号。RK3588示例配置已根据2026-09-02实物复验写入显式`R_BC`
和`verified=true`；其他相机或重新安装后仍必须完成以下方向检查，不能直接复制验证标志：
...
yaw重算时残差1.74 deg。2026-09-02两轮固定目标动作复验进一步确认各姿态惯性LOS残差均小于
5 deg，并确认RAW_IMU转FRD为`axis_sign=[+1,-1,+1]`：独立pitch轮相关系数`+0.969`；后续
20 Hz复测9/9个Yaw事件积分与航向变化同号。Yaw倾斜补偿精度和硬件曝光时间戳仍未关闭，因此
该结果不构成有桨许可。板端现有
`*.local.json`和批准文件未被本次部署覆盖；下一次无桨RC测试必须从新示例同步已验证字段并用新
快照重新生成批准文件，届时旧批准会因参数SHA256变化自动失效。
当前跟踪配置哈希为：LOG_ONLY
`9e5dbb9de21ca59434c1f53a020a76b7f27d1089721d17ec16c96d2ef11d16c9`，无桨受限配置
`c6e2fc45399a79ed6143cbdca8eb1465e4a75229d5bdc2228edd5249e9a503bd`。完整报告见
`doc/BETAFLIGHT_RBC_GYRO_VALIDATION_20260902.md`。
...
MSP、解析或相机错误。该低延迟版本当时的配置SHA256为
`259f9793fb4c74750275595ced8a07ff33986de3b0044d0cce9158c9dec50a99`；2026-09-02写入显式外参和
RAW_IMU Y轴修正后哈希已更新，见本节后续验证记录。该优化只适用于LOG_ONLY数据采集；GPS
有效性依赖室外环境，状态源单UART及候选离线非放行问题仍未解决。
