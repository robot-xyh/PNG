...
`[0,0,-1]`。本机已通过固定目标动作确认
`R_BC=[[0,1,0],[1,0,0],[0,0,-1]]`，正交、det=+1且各姿态惯性LOS残差小于5 deg；RAW_IMU
转FRD使用`axis_sign=[+1,-1,+1]`，20 Hz复测9/9个Yaw事件同号。该结果只绑定当前相机安装和
`BTFL 25.12.2 / API 1.47`，拆装或升级后必须复验。批准工具要求上述显式矩阵和实物验证记录。
时间戳不等于可信曝光时刻；需使用V4L2单调
