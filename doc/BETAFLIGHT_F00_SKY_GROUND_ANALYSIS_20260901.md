...
| 外参状态 | `verified=false`，光轴误差 90 deg，禁止据此批准主动控制 |
...
日志中的 154 个 `guidance_valid/sp_valid` 是离线算法候选，不代表飞控收到控制。外参仍未验证，
这些结果也不能用于证明 LOS/指令物理方向正确。
...
| 主动控制放行 | 未通过 | 外参未验证，且非目标航空器可进入制导链路 |
