...
VEHICLE_IP_FIELDS = {"LocalHostIp", "ControlIp", "UdpIp"}
...
def _rewrite_vehicle_loopback_ips(settings: dict[str, Any], host: str) -> list[str]:
    notes: list[str] = []
    vehicles = settings.get("Vehicles", {})
    if not isinstance(vehicles, dict):
        return notes
    for vehicle_name, vehicle in vehicles.items():
        if not isinstance(vehicle, dict):
            continue
        for field in sorted(VEHICLE_IP_FIELDS):
            value = vehicle.get(field)
            ...
            if value == host:
                continue
            vehicle[field] = host
            notes.append(f"{vehicle_name}.{field} {value or '(empty)'} -> {host}")
    return notes
...
def main() -> int:
    ...
    parser.add_argument(
        "--host",
        default=os.environ.get("AIRSIM_BIND_HOST", DEFAULT_HOST),
        help="Loopback host used for this repository's AirSim RPC and PX4 settings.",
    )
    ...
    changed = False
    notes: list[str] = []
    host_notes = _rewrite_vehicle_loopback_ips(settings, host)
    if host_notes:
