...
SCRIPT_BY_SCENARIO = {
    "truth": PROJECT_ROOT / "examples" / "run_airsim_truth_png.py",
    "gimbal": PROJECT_ROOT / "examples" / "run_airsim_gimbal_vision_png.py",
    "strapdown": PROJECT_ROOT / "examples" / "run_airsim_strapdown_vision_png.py",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-test PNG variants in AirSim.")
    parser.add_argument("--scenario", choices=sorted(SCRIPT_BY_SCENARIO), required=True)
    parser.add_argument("--ranges", nargs="+", type=float, default=[30.0, 40.0, 50.0, 60.0])
    parser.add_argument("--altitude-offsets", nargs="+", type=float, default=[20.0])
    ...
    parser.add_argument("--trajectory-dir", default=str(PROJECT_ROOT / "logs" / "png_accuracy"))
    ...
    parser.add_argument("--summarize-prefix", default="")
...
def _case_name(prefix: str, scenario: str, range_m: float, alt_m: float) -> str:
    base = prefix or time.strftime(f"{scenario}_accuracy_%Y%m%d-%H%M%S")
...
def _case_args(args: argparse.Namespace, case_prefix: str, range_m: float, alt_m: float, reset: bool) -> list[str]:
    script = SCRIPT_BY_SCENARIO[args.scenario]
    command = [
        sys.executable,
        str(script),
        "--enable-motion",
    ...
    if args.scenario in {"gimbal", "strapdown"}:
        command.extend(["--no-show-window", "--no-record-preview"])
...
def _summarize_existing(args: argparse.Namespace, out_dir: Path) -> list[dict]:
    rows = []
...
def main() -> None:
    ...
    batch_prefix = args.prefix or time.strftime(f"{args.scenario}_accuracy_%Y%m%d-%H%M%S")

    if args.summarize_prefix:
        summary_rows = _summarize_existing(args, out_dir)
        summary_path = out_dir / f"{args.summarize_prefix}_summary.csv"
        _write_summary(summary_rows, summary_path)
        print(f"png_accuracy_summary={summary_path}")
        return
    ...
    for range_m in args.ranges:
        for alt_m in args.altitude_offsets:
            case_prefix = _case_name(batch_prefix, args.scenario, range_m, alt_m)
            reset = first or not args.no_reset_between_runs
            first = False
            command = _case_args(args, case_prefix, range_m, alt_m, reset)
            print(f"\n=== scenario={args.scenario} case={case_prefix} range={range_m:.1f}m alt_offset={alt_m:.1f}m ===", flush=True)
            ...
            print(
                ...
                f"max_load_fd_g={summary.get('max_load_factor_fd_g')}, "
    ...
    _write_summary(summary_rows, summary_path)
    print(f"\npng_accuracy_summary={summary_path}")
